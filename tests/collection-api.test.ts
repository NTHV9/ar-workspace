...
it('ignores forged eligible flags and rejects authoritative child result',async()=>{
 const calls:unknown[]=[];vi.stubGlobal('fetch',async(url:string,options:RequestInit)=>{if(url.endsWith('/auth/v1/user'))return Response.json({id:'00000000-0000-4000-8000-000000000001',email:'ar@katathani.com',email_confirmed_at:'2026-09-09'});calls.push(JSON.parse(String(options.body)));return Response.json(false);});
...
it('fails closed when database validation is unavailable',async()=>{
 vi.stubGlobal('fetch',async(url:string)=>url.endsWith('/auth/v1/user')?Response.json({id:'00000000-0000-4000-8000-000000000001',email:'ar@katathani.com',email_confirmed_at:'2026-09-09'}):new Response('',{status:503}));

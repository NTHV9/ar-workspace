...
type Route='paymentDetail'|'paymentHistory'|'invoiceHistory'|'invoiceDetail'|'invoiceBack';
function setup(options:{count?:number;debit?:boolean;zero?:boolean;sparse?:boolean;mutate?:(route:Route,count:number,value:Obj,url:URL)=>Obj}={}){
 ...
 const reads:Record<Route,number>={paymentDetail:0,paymentHistory:0,invoiceHistory:0,invoiceDetail:0,invoiceBack:0},calls:URL[]=[];
 ...
 const reader=new OperaReader({origin:'https://opera.synthetic.invalid',appKey:'synthetic',hotelId:'KAT'},async()=>'synthetic',async request=>{
  ...
  if(url.pathname.includes('/invoicePayments/accounts/')){
...
describe('payment history reaches old invoices independently of Bill Date',()=>{
 it('corroborates both directions, keeps paid-zero/old invoice facts and strips private descriptors',async()=>{
  ...
  expect(JSON.stringify(result)).not.toContain('Synthetic private guest');expect(h.reads).toEqual({paymentDetail:2,paymentHistory:2,invoiceHistory:2,invoiceDetail:4,invoiceBack:2});
  for(const url of h.calls.filter(url=>url.pathname.includes('/invoicePayments/'))){expect(url.searchParams.get('inclZeroBalance')).toBe('true');expect(url.searchParams.has('start')).toBe(false);expect(url.searchParams.has('end')).toBe(false);}
 });
 ...
 it('can verify an explicitly unallocated payment with two empty histories and stable detail',async()=>{
  const h=setup({zero:true}),result=await readPaymentApplications(h.reader,h.payment);expect(result.invoices).toEqual([]);expect(result.links).toEqual([]);expect(h.reads).toEqual({paymentDetail:2,paymentHistory:2,invoiceHistory:0,invoiceDetail:0,invoiceBack:0});
 });
 it('batches printed numbers at 20 and rechecks both batches',async()=>{
  ...
  expect(h.calls.filter(url=>url.pathname.includes('/invoicePayments/')).map(url=>url.searchParams.getAll('invoiceNo').length)).toEqual([20,20,1,1]);
 });
...
describe('payment application proof fails closed',()=>{
 ...
 it('fails closed for reported transfer payments even if both source reads agree',async()=>{
  const h=setup({mutate:(route,_count,value)=>{if(route==='paymentDetail')groupRows(value,'payments')[0].transferredIn=true;return value;}});h.payment.transferredIn=true;h.payment.transfer='in';await expect(readPaymentApplications(h.reader,h.payment)).rejects.toMatchObject({stage:'financial_payment_mapping_payment_facts'});
 });

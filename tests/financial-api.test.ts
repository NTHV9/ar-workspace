...
it.each([2,3])('version%s financial imports use the background queue instead of interactive document capacity',async(stepsVersion)=>{
 const interactive={create:vi.fn(),get:vi.fn()},background={create:vi.fn().mockResolvedValue({}),get:vi.fn()};
 vi.stubGlobal('fetch',async()=>Response.json({id:actor,stepsVersion,status:'queued',created:false,hotel:'KAT'}));
...
it.each([2,3])('a missing background binding does not consume document capacity for a version%s job',async(stepsVersion)=>{
 const create=vi.fn();vi.stubGlobal('fetch',async()=>Response.json({id:actor,stepsVersion,status:'queued',created:false,hotel:'KAT'}));

...
const invoice=(id:number,open=80)=>({transactionNo:id,invoiceNo:id+100,folioNo:id+200,guestName:'Example',transactionDate:'2026-09-01',amount:money(100),payments:money(open-100),balance:money(open),reference:'VCH-'+id,compressed:false});
...
it("rejects inconsistent payment signs rather than inferring a sign",()=>{const bad=structuredClone(raw);bad.accountDetails.invoices[0].payments=money(20);expect(()=>statementModel(bad,[manifest[0]],"2026-09-09")).toThrow();});

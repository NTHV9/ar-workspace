...
test('Preview defaults to width and allows saving without visiting every output sheet',async({page})=>{
 ...
 await preview.getByRole('button',{name:'Save reviewed PDFs privately',exact:true}).click();await expect(preview.getByRole('button',{name:'Saved',exact:true})).toBeVisible();
 const saved=await page.evaluate(async()=>{const api=(window as any).pdfTest;const pages=await Promise.all(api.saved.files.map(async(f:any)=>(await api.PDFDocument.load(f.bytes)).getPageCount()));return {files:pages.length,pages:pages.reduce((a:number,b:number)=>a+b,0)};});expect(saved).toEqual({files:3,pages:4});
});

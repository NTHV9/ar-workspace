...
test('late source export and save completion do not mark newer work reviewed or saved',async({page})=>{
 ...
 await page.evaluate(()=>{const api=(window as any).pdfTest;api.saveWait=new Promise(resolve=>api.releaseSave=resolve);});await page.getByRole('button',{name:'Open mandatory Preview'}).click();await page.getByRole('checkbox').check();await page.getByRole('button',{name:'Save reviewed PDFs privately'}).click();await expect(page.getByRole('button',{name:'Close final preview'})).toBeDisabled();await expect(page.locator('.pdf-layout')).toHaveAttribute('inert','');await page.evaluate(()=>(window as any).pdfTest.releaseSave());await expect(page.getByRole('button',{name:'Saved',exact:true})).toBeVisible();await page.getByRole('button',{name:'Close final preview'}).click();await expect(page.getByRole('button',{name:'Move note layer'})).toHaveCount(0);
});
...
test('direct source editing keeps source formatting and adds/removes text lines',async({page})=>{
 ...
 const text=page.getByRole('textbox',{name:'Layer text',exact:true});await expect(text).toBeFocused();
...
test('native line and a selected table area can be dragged with exact page edits',async({page})=>{
 ...
 await page.getByRole('button',{name:'Save draft',exact:true}).click();snapshot=await page.evaluate(()=>(window as any).pdfTest.draft.pages[0]);expect(snapshot.rowEdits).toHaveLength(2);expect(snapshot.rowEdits[1].width).toBeGreaterThan(500);expect(snapshot.rowEdits[1].dy).toBeGreaterThan(10);
});

test('a multiline row cannot delete neighboring source text',async({page})=>{
 await page.goto('/tests/browser/pdf-editor-harness.html');await page.getByRole('button',{name:'Edit original text: SYNTHETIC ROW A',exact:true}).click();await page.getByRole('textbox',{name:'Layer text',exact:true}).fill('SYNTHETIC ROW A\nSYNTHETIC ROW A');
 await page.getByRole('button',{name:'Delete row',exact:true}).click();await expect(page.getByRole('alert').first()).toContainText('crosses');await expect(page.getByRole('button',{name:'Edit original text: SYNTHETIC ROW B',exact:true})).toBeVisible();
 await page.getByRole('button',{name:'Save draft',exact:true}).click();expect(await page.evaluate(()=>(window as any).pdfTest.draft.pages[0].rowEdits??[])).toEqual([]);
});
...
test('inserted cells retain row controls and clone all columns after a font change',async({page})=>{
 ...
 expect(p.rowEdits).toHaveLength(2);expect(p.layers.filter((l:any)=>l.tableRow===p.rowEdits[0].id)).toHaveLength(3);expect(p.layers.filter((l:any)=>l.tableRow===p.rowEdits[1].id)).toHaveLength(3);expect(p.layers.filter((l:any)=>l.tableRow===p.rowEdits[1].id&&l.kind==='text')).toHaveLength(1);
});

test('row insertion keeps vertical rules and never clips bottom content',async({page})=>{
 await page.goto('/tests/browser/pdf-editor-harness.html');await page.waitForFunction(()=>(window as any).pdfTest);
 const proof=await page.evaluate(()=>{const api=(window as any).pdfTest,c=document.createElement('canvas');c.width=120;c.height=120;const ctx=c.getContext('2d')!;ctx.fillStyle='white';ctx.fillRect(0,0,120,120);ctx.fillStyle='#143875';ctx.fillRect(20,10,2,90);api.applySourceRowEdits(c,[{id:'row',kind:'insert',y:50,height:10}],1);const continued=Array.from(ctx.getImageData(20,50,1,10).data).every((v,i)=>v===[20,56,117,255][i%4]);ctx.fillStyle='black';ctx.fillRect(60,116,5,3);let blocked=false;try{api.applySourceRowEdits(c,[{id:'bad',kind:'insert',y:60,height:10}],1);}catch{blocked=true;}return {continued,blocked,bottom:[...ctx.getImageData(60,117,1,1).data]};});
 expect(proof).toEqual({continued:true,blocked:true,bottom:[0,0,0,255]});
});

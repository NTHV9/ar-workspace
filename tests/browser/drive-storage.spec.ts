...
test('leaving Storage cancels Picker and ignores its late selection',async({page})=>{
 const controls=await setup(page);controls.status.pickerConfigured=true;await mockPicker(page);await page.goto('/?storage=1');await page.getByRole('button',{name:'Choose confirmed folder',exact:true}).click();await page.getByRole('navigation',{name:'Main navigation'}).getByRole('button',{name:'Portfolio',exact:true}).click();await expect(page.getByRole('navigation',{name:'Main navigation'}).getByRole('button',{name:'Portfolio',exact:true})).toHaveClass('active');await expect(page.getByRole('heading',{name:'Storage',exact:true})).toHaveCount(0);await page.evaluate(id=>(window as any).__syntheticPicker.pickerCallback({action:'picked',docs:[{id}]}),folderId);expect(controls.folderWrites).toHaveLength(0);expect(await page.evaluate(()=>(window as any).__syntheticPicker.disposeCount)).toBe(1);
});

...
async function setup(page:Page,conflict=false){
 ...
 await page.route('**/api/**',async r=>{const q=r.request(),p=new URL(q.url()).pathname;calls.push(q.method()+' '+p);
  ...
  if(p==='/api/gmail/status')return r.fulfill({json:{configured:true,connected:true,email:'ar@katathani.com',maxAttachmentBytes:10485760}});
  ...
  if(p===`/api/email/${draftId}/gmail-draft`)return r.fulfill({json:{state:'created',created:true}});
...
for(const width of [1440,1280])test(`email workspace ${width}: saved message and explicit Gmail draft`,async({page})=>{
 ...
 await page.getByRole('button',{name:'Create Gmail draft',exact:true}).click();await expect(page.getByRole('status')).toContainText('Gmail confirmed draft creation');await expect(page.getByRole('button',{name:'Create Gmail draft',exact:true})).toBeDisabled();expect(calls.filter(c=>c.endsWith('/gmail-draft'))).toHaveLength(1);
});

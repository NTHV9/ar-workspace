...
export async function reviewPreviewPages(page:Page){
 ...
 await expect(preview).toBeVisible();await expect(preview.getByRole('checkbox')).toHaveCount(0);await expect(preview.getByText('I reviewed every output page',{exact:false})).toHaveCount(0);await expect(files.first()).toBeVisible();
 ...
 await expect(preview.locator('.pdf-preview-actions .pdf-primary'),JSON.stringify(await preview.locator('.pdf-final-viewer').evaluate(e=>({index:e.getAttribute('data-page-index'),viewed:e.getAttribute('data-viewed'),count:e.getAttribute('data-count'),state:e.querySelector('.pdf-final-sheet')?.getAttribute('data-render-state')})))).toBeEnabled();
}

...
test('opening an account and returning restores report mode, scope, dates and page',async({page})=>{
 const calls=await setup(page);await page.goto('/?reports=1&hotel=KAT');
 await page.getByRole('tab',{name:'Verified sent activity'}).click();
 await page.getByLabel('Report account type').selectOption('OTA');
 await page.getByLabel('Report account',{exact:true}).selectOption(JSON.stringify(['KAT',account.id]));
 await page.getByLabel('Report activity kind').selectOption('First billing');
 await page.getByLabel('Report from date').fill('2026-09-01');await page.getByLabel('Report through date').fill('2026-09-10');
 await page.getByRole('button',{name:'Next report page',exact:true}).click();
 await page.getByRole('button',{name:'View history SYNTH-LAST'}).click();
 await page.getByRole('button',{name:'Open current account',exact:true}).click();
 ...
 await expect(page.getByRole('tab',{name:'Verified sent activity'})).toHaveAttribute('aria-selected','true');
 await expect(page.getByLabel('Report account type')).toHaveValue('OTA');
 await expect(page.getByLabel('Report account',{exact:true})).toHaveValue(JSON.stringify(['KAT',account.id]));
 await expect(page.getByLabel('Report activity kind')).toHaveValue('First billing');
 await expect(page.getByLabel('Report from date')).toHaveValue('2026-09-01');await expect(page.getByLabel('Report through date')).toHaveValue('2026-09-10');
 await expect(page.locator('.reports-table tbody tr')).toHaveCount(1);await expect(page.locator('.reports-table')).toContainText('SYNTH-LAST');
 const returned=calls.slice(beforeReturn).filter(c=>c.startsWith('GET /api/reports/activity?')&&!c.includes('invoice='));
 expect(returned.length).toBeGreaterThan(0);
 expect(returned.every(c=>{const q=new URL('https://synthetic.test'+c.slice(4)).searchParams;return q.get('hotel')==='KAT'&&q.get('type')==='OTA'&&q.get('account')===account.id&&q.get('kind')==='First billing'&&q.get('from')==='2026-09-01'&&q.get('to')==='2026-09-10'&&q.get('page')==='1';})).toBe(true);
 await page.getByRole('button',{name:'TSK',exact:true}).click();

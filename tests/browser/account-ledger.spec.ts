...
async function setup(page:Page){
 ...
 await page.route('**/api/**',r=>{const p=new URL(r.request().url()).pathname;
  ...
  if(p==='/api/portfolio')return r.fulfill({json:{status:'connected',accounts:[{hotel:'KAT',id:'SYN-A',name:'Synthetic Ledger Account',type:'Agent',open:800,over90:300,items:8,agingBuckets:buckets}],refresh:{running:false,hotels:[]}}});
  if(p==='/api/accounts/KAT/SYN-A')return r.fulfill({json:{invoices:entries.map(e=>({...e,hotel:'KAT',account_id:'SYN-A',invoice_no:'SYN-'+e.id,folio_no:'FOL-'+e.id,transaction_date:'2026-09-01',original:100,open:100,collection_role:'standalone',collection_selectable:true,verification_state:'verified'}))}});

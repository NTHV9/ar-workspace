...
test('resets pagination and retains the preference when returning from Period analysis',async({page})=>{
 ...
 await page.getByRole('button',{name:'Period analysis',exact:true}).click();await page.getByRole('button',{name:'Current Aging · Khao Lak',exact:true}).click();await expect(page.getByRole('checkbox',{name:'Hide empty accounts',exact:true})).toBeChecked();await expect(page.locator('.aging-pagination')).toContainText('6 matched accounts');
});

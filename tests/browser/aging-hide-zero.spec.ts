...
for(const region of ['phuket','khao-lak'] as const)for(const width of [1440,1280,390])test(`hide zero accounts is reversible and scoped in ${region} at ${width}`,async({page})=>{
 ...
 await page.getByLabel('Current Account Type',{exact:true}).selectOption('Agent');const table=page.getByRole('table',{name:'Current source aging comparison'}),toggle=page.getByRole('checkbox',{name:'Hide empty accounts',exact:true});
...
test('keeps unknown rows while counts load and hides only after proof arrives',async({page})=>{
 const c=await setup(page,'phuket',{delayed:true});await page.goto('/?dashboard=1&dashboardView=aging');await page.getByLabel('Current Account Type',{exact:true}).selectOption('Agent');const table=page.getByRole('table',{name:'Current source aging comparison'});await page.getByRole('checkbox',{name:'Hide empty accounts',exact:true}).check();await expect(table.locator('tbody[data-aging-group]')).toHaveCount(8);c.release();await expect(table.locator('tbody[data-aging-group]')).toHaveCount(6);
});
test('keeps all rows when invoice verification cannot be loaded',async({page})=>{
 await setup(page,'khao-lak',{unavailable:true});await page.goto('/?region=khao-lak&dashboard=1&dashboardView=aging');await page.getByLabel('Current Account Type',{exact:true}).selectOption('Agent');await expect(page.getByRole('button',{name:'Retry invoice counts',exact:true})).toBeVisible();await page.getByRole('checkbox',{name:'Hide empty accounts',exact:true}).check();await expect(page.getByRole('table',{name:'Current source aging comparison'}).locator('tbody[data-aging-group]')).toHaveCount(8);
});
test('resets pagination and retains the preference when returning from Period analysis',async({page})=>{
 ...
 await page.locator('.aging-pagination').getByRole('button',{name:'Next',exact:true}).click();await expect(page.locator('.aging-pagination')).toContainText('page 2 of 2');await page.getByRole('checkbox',{name:'Hide empty accounts',exact:true}).check();await expect(page.locator('.aging-pagination')).toContainText('page 1 of 1');
 await page.getByRole('navigation',{name:'Main navigation'}).getByRole('button',{name:'Dashboard',exact:true}).click();await page.getByRole('navigation',{name:'Main navigation'}).getByRole('button',{name:'Aging',exact:true}).click();await expect(page.getByRole('checkbox',{name:'Hide empty accounts',exact:true})).toBeChecked();await expect(page.locator('.aging-pagination')).toContainText('6 matched accounts');
});

import type {QueueRow} from '../../src/CollectionQueue';
...
test('Khao Lak can prepare verified invoices before billing setup without email',async({page})=>{
 const data=[{...rows()[5],hotel:'TLKL'}];const calls=await setup(page,false,data);
 await page.goto('/?collections=1&region=khao-lak');
 await page.getByLabel('Queue select INV-6',{exact:true}).check();
 await page.getByRole('button',{name:'Prepare documents',exact:true}).click();
 await expect(page.getByRole('dialog').getByRole('combobox',{name:'Purpose',exact:true})).toHaveValue('billing');
 await expect(page.getByRole('dialog').getByRole('combobox',{name:'Document content',exact:true})).toContainText('Invoices');
 await expect(page.getByRole('dialog')).not.toContainText('Folios');
...
for(const width of [1440,1280,390])test(`collection queue ${width}: correct stage labels, filters and scoped selection`,async({page})=>{const calls=await setup(page);await page.setViewportSize({width,height:width===1440?900:width===1280?800:844});await page.goto('/?collections=1');await expect(page.getByRole('heading',{name:'Billing & Collection Queue',exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Harbor Tours · Synthetic',exact:true})).toBeVisible();await page.screenshot({path:`evidence/collections-clean-copy-${width}.png`,fullPage:true});
 await page.getByLabel('Queue stage',{exact:true}).selectOption('Follow 2');await expect(page.locator('.queue-work tbody tr')).toHaveCount(1);await expect(page.locator('.queue-work tbody')).toContainText('Follow-up 2');await page.getByRole('button',{name:'Harbor Tours · Synthetic',exact:true}).click();await page.getByLabel('Queue select INV-2',{exact:true}).check();if(width===390)await page.screenshot({path:'evidence/collections-clean-copy-390-drawer.png'});await page.getByRole('button',{name:'Prepare documents',exact:true}).click();await expect(page.getByRole('dialog').getByRole('combobox',{name:'Purpose',exact:true})).toHaveValue('collection');await page.getByRole('dialog').getByRole('button',{name:'Cancel',exact:true}).click();if(width===390)await page.getByRole('button',{name:'Close queue details',exact:true}).click();expect(calls.some(c=>c.includes('/send')||c.includes('/gmail-draft')||c==='POST /api/documents')).toBe(false);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
});
...
test('configured Billing not required identifies the missing credit term in queue details',async({page})=>{
 const sample=rows()[4];const data=[{...sample,workflow:{...sample.workflow!,credit_term:null,due_date:null}}];
 const calls=await setup(page,false,data);await page.goto('/?collections=1');await expect(page.locator('.queue-work tbody')).toContainText('Credit term needed');await expect(page.locator('.queue-detail')).toContainText('No reminders sent');await expect(page.locator('.queue-detail')).toContainText('Set the credit term to determine the due date');await expect(page.getByRole('button',{name:'Prepare documents',exact:true})).toBeDisabled();expect(calls.some(c=>c.includes('/send')||c.includes('/gmail-draft')||c==='POST /api/documents')).toBe(false);
});

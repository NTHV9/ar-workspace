...
it('preserves a negative AR rebate without inventing a VAT adjustment',()=>{const p=arAdjustmentPacket();p.manifest.open=3215;p.invoice.amount=money(3215);p.invoice.balance=money(3215);p.postings.invoicePostingsDetails[1].debitAmount=money(-100);p.arDetails[0].response.transactions[0].debitAmount=money(-100);p.arDetails[0].response.transactions[0].postedAmount=money(-100);const model=invoiceModel(p,now);expect(model.gross).toBe(321500);expect(model.nonTaxable).toBe(-10000);expect(model.vat).toBe(invoiceModel(packet(1),now).vat);});

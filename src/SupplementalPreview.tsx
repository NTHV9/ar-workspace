...
export default function SupplementalPreview({draftId,file,token,onClose}:{draftId:string;file:EmailAttachment;token:string;onClose:()=>void}){
 ...
 useEffect(()=>{region.current?.focus();let active=true;let imageUrl='';let destroy:()=>Promise<void>=async()=>{};const controller=new AbortController();setError('');setStatus('Loading private attachment…');setPdf(null);setImage('');setPage(1);
  void(async()=>{let failure='This attachment could not be loaded or rendered. Close and try again.';try{
   const r=await fetch(`/api/email/${draftId}/attachments/${file.id}`,{headers:{Authorization:`Bearer ${token}`},signal:controller.signal});if(!r.ok){if(r.status===410){const detail:unknown=await r.json().catch(()=>null);if(detail&&typeof detail==='object'&&'error'in detail&&detail.error==='storage_file_expired')failure=operationMessages.storage_file_expired+' Start a new document preparation and attach the original supplemental file again.';}throw Error();}
  ...
  }catch{if(active){setError(failure);setStatus('');}}})();

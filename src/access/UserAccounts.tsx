...
import {normalizeLogin,validInitialPassword} from './identity';
...
type FormMode='create'|'approve'|'edit';
const blank=()=>({login:'',regions:['phuket'] as RegionId[],active:true,revision:0});
...
export default function UserAccounts({token,onDirtyChange}:{token:string;onDirtyChange?:(dirty:boolean)=>void}){
 ...
 useEffect(()=>{setBusy(false);setPassword('');setConfirmation('');},[token]);
 const reset=(next:FormMode='create')=>{setMode(next);setDraft(blank());setSelected(null);setDirty(false);setPassword('');setConfirmation('');setPendingCreate(null);pending.current=null;setError('');loginField.current?.focus();};
 const switchMode=(next:FormMode)=>{if(busy||dirty&&!window.confirm('Discard unsaved user changes?'))return;reset(next);setStatus('');};
 const edit=(row:AccessMember,create=false)=>{
  if(busy||dirty&&!window.confirm('Discard unsaved user changes?'))return;
  reset(create?'create':'edit');setSelected(row);setDraft({login:memberLogin(row),regions:[...row.regions],active:row.active,revision:row.revision});setStatus('');
 };
 ...
 const settled=(result:Result,commandId:string)=>{
  ...
  }else{setStatus(result.kind==='delete'?'User deleted. Business history is retained.':result.kind==='create'?'User created. Share the initial password securely.':'Access saved. No invitation email was sent.');reset();}
  refresh();
 };
 const save=async()=>{
  if(busy||pendingCreate||!draft.regions.length)return;setError('');setStatus('');
  try{normalizeLogin(draft.login);if(mode==='create'&&(!validInitialPassword(password)||password!==confirmation))throw Error(password!==confirmation?'Passwords do not match.':messages.access_password_invalid);}catch(e){setError(e instanceof Error?(messages[e.message]??e.message):messages.access_login_invalid);return;}
  ...
  try{
   let result:Result;
   if(mode==='create')result=await post(token,'/api/access/users/create',{commandId:cmd.id,login:draft.login.trim().toLowerCase(),password,regions:draft.regions,revision:draft.revision});
   else if(mode==='edit'&&selected?.memberId)result=await post(token,`/api/access/users/${selected.memberId}/edit`,{commandId:cmd.id,regions:draft.regions,active:draft.active,revision:draft.revision});
   else result=await post(token,'/api/access/users',{commandId:cmd.id,email:draft.login.trim().toLowerCase(),regions:draft.regions,active:draft.active,revision:draft.revision});
  ...
  }catch(e){if(lifetime.current===version){setError(e instanceof Error?e.message:'User request failed.');if(mode==='create'&&(!(e instanceof AccessError)||['access_auth_unavailable','access_auth_unverified','access_unavailable','unavailable'].includes(e.code)))setPendingCreate(cmd.id);else pending.current=null;}}
  finally{setPassword('');setConfirmation('');if(lifetime.current===version)setBusy(false);}
 };
 ...
 return <main className="page user-access">
  ...
  <section className="panel access-form">
   <div className="access-form-heading"><h2>{mode==='edit'?'Edit user access':mode==='approve'?'Add approved user':selected?'Set up login account':'Create user'}</h2>{mode!=='edit'&&<div className="access-mode" aria-label="User setup method"><button type="button" aria-pressed={mode==='create'} disabled={busy} onClick={()=>switchMode('create')}>Username / password</button><button type="button" aria-pressed={mode==='approve'} disabled={busy} onClick={()=>switchMode('approve')}>Approve email only</button></div>}</div>
   <form onSubmit={e=>{e.preventDefault();void save();}}>
    <label className="access-email">{mode==='approve'||mode==='edit'&&!selected?.username?'Email':'Username or email'}<input ref={loginField} type={mode==='approve'?'email':'text'} autoComplete="off" maxLength={254} required readOnly={mode==='edit'||!!selected} disabled={busy||!!pendingCreate} value={draft.login} onChange={e=>{setDraft({...draft,login:e.target.value});setDirty(true);}} placeholder={mode==='approve'?'name@company.com':'phuket.ar or name@company.com'}/></label>
    ...
    {mode==='create'&&<div className="access-passwords"><label>Password<input type="password" autoComplete="new-password" minLength={12} maxLength={72} required disabled={busy||!!pendingCreate} value={password} onChange={e=>{setPassword(e.target.value);setDirty(true);}}/></label><label>Confirm password<input type="password" autoComplete="new-password" required disabled={busy||!!pendingCreate} value={confirmation} onChange={e=>{setConfirmation(e.target.value);setDirty(true);}}/></label></div>}
    <div className="access-form-actions">{pendingCreate?<button type="button" className="primary-button" disabled={busy} onClick={()=>void check(pendingCreate)}>Check creation</button>:<button className="primary-button" disabled={busy||!draft.login.trim()||!draft.regions.length}><UserPlus size={15}/>{busy?'Saving…':mode==='edit'?'Save access':mode==='approve'?'Approve access':'Create user'}</button>}{(mode==='edit'||dirty||pendingCreate||selected)&&<button type="button" disabled={busy} onClick={()=>reset()}>Cancel</button>}</div>
   </form>
   <p className="access-help">{mode==='create'?'Use a short username without an email, or enter an email to also allow Google sign-in. Use at least 12 password characters. Passwords are not shown again after creation.':'Approved email users can sign in with Google or create a web app password themselves. Approving access does not send an invitation.'} Access does not change the sending mailbox.</p>
  </section>
  ...
  <section className="panel access-list"><header><h2>Users{data&&<small>{data.total}</small>}</h2><label className="access-search"><Search size={15}/><input type="search" aria-label="Search users" disabled={busy} placeholder="Search username or email" value={search} onChange={e=>{setSearch(e.target.value);setPage(0);}}/></label></header>
   {loading?<p role="status" className="access-empty">Loading users…</p>:data&&<><div className="access-table-scroll"><table><thead><tr><th>Username / email</th><th>Regions</th><th>Status</th><th>Sign-in</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{data.rows.map(row=><tr key={row.memberId??memberLogin(row)}>
    <td><strong>{memberLogin(row)}</strong>{row.administrator&&<small>Administrator · protected</small>}</td><td>{row.regions.map(regionLabel).join(' · ')}</td>
    ...
    <td>{row.username?'Username / password':row.registered?'Email / Google':row.hasLoginAccount?'Email confirmation pending':'Awaiting first sign-in'}</td>
    <td>{row.administrator?<span className="access-locked"><ShieldCheck size={16}/> Both regions</span>:<div className="access-row-actions">{row.pendingCommand?<button disabled={busy} onClick={()=>void check(row.pendingCommand!)}>{row.setupState==='deleting'?'Check deletion':'Check setup'}</button>:<><button aria-label={'Edit access for '+memberLogin(row)} disabled={busy} onClick={()=>edit(row)}><Pencil size={14}/> Edit</button>{!row.registered&&!row.hasLoginAccount&&row.memberId&&<button disabled={busy} onClick={()=>edit(row,true)}>Set up password</button>}</>}{row.memberId&&row.setupState!=='deleting'&&<button className="access-delete-button" aria-label={'Delete '+memberLogin(row)} disabled={busy} onClick={()=>{setDeleteTarget(row);setDeleteError('');}}><Trash2 size={14}/> Delete</button>}</div>}</td>

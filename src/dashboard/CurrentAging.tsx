...
function AgingWorkspace({region='phuket',revision=0,token,hotel,accounts,refresh,onOpenInvoice,initialContext,onContextChange}:Props){
 const agingHotels=comparisonHotels(accounts,region);
 ...
 const counts=useSource(accounts.length?'/api/dashboard/aging-invoices'+(region==='khao-lak'?'?region=khao-lak'+(isHotelId(hotel)?'&hotel='+hotel:''):isHotelId(hotel)?'?hotel='+hotel:''):null,token,agingSourceRevision(revision+countRevision+agingCatalogRevision(accounts),refresh??undefined),agingInvoicesResult,true);
 ...
 const context=useMemo<AgingContext>(()=>({hotel,allAccounts,over90,type,accountKey,search,page,sort,bucketKey:effectiveBucketKey,invoiceHotel,invoiceView,invoicePage,invoiceDescending,hideZero,columnVisibility,comparisonView,countSelection,countFilters}),[hotel,allAccounts,over90,type,accountKey,search,page,sort,effectiveBucketKey,invoiceHotel,invoiceView,invoicePage,invoiceDescending,hideZero,columnVisibility,comparisonView,countSelection,countFilters]);
 ...
 return <section className="current-aging" aria-labelledby="current-aging-title">
  <header className="aging-heading"><div><h2 id="current-aging-title">Current Aging</h2><p>Latest saved OPERA balances</p></div><span className="aging-scope">{hotel==='All'?regionHotels(region).join(' + '):hotel} · THB</span></header>
  ...
  <nav className="aging-breadcrumbs" aria-label="Current Aging drilldown"><button onClick={()=>chooseType()} disabled={typeView}>All account types</button>{type!==undefined&&<><ChevronRight size={13}/><button onClick={()=>setAccountKey(undefined)} disabled={!selected}>{type}</button></>}{selected&&<><ChevronRight size={13}/><span>{selected.name}</span></>}</nav>
  ...
  {selected&&<button className="aging-back" onClick={()=>setAccountKey(undefined)}><ArrowLeft size={14}/> Back to {type??'all'} accounts</button>}
  ...
  <div className="aging-table-toolbar"><div><h3>All-range comparison <span>· {selected?'account':typeView?'account types':'matched accounts'}</span></h3><p>{visibleColumns.length} of {columns.length} source ranges shown. Click an amount to view its invoices.</p></div>

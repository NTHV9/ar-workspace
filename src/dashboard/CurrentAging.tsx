...
import {filterSortInvoices,invoiceColumns,type InvoiceSortKey} from './invoice-list';
...
function AgingWorkspace({region='phuket',revision=0,token,hotel,accounts:catalog,refresh,onOpenInvoice,initialContext,onContextChange}:Props){
 ...
 const [invoiceSort,setInvoiceSort]=useState<InvoiceSortKey>(initial?.invoiceSort??'open'),[invoiceSearch,setInvoiceSearch]=useState(initial?.invoiceSearch??'');
 ...
 const context=useMemo<AgingContext>(()=>({hotel,invoiceSort,invoiceSearch,accountReturn,allAccounts,over90,type,accountKey,search,page,sort,bucketKey:effectiveBucketKey,invoiceHotel,invoiceView,invoicePage,invoiceDescending,hideZero,columnVisibility,comparisonView,countSelection,countFilters}),[hotel,invoiceSort,invoiceSearch,accountReturn,allAccounts,over90,type,accountKey,search,page,sort,effectiveBucketKey,invoiceHotel,invoiceView,invoicePage,invoiceDescending,hideZero,columnVisibility,comparisonView,countSelection,countFilters]);
 ...
 const drill=(row:AgingComparisonRow,h:AgingHotel='Total',bucket?:AgingBucket)=>{
  if(!selected&&!typeView){setInvoiceSearch('');const node=[...document.querySelectorAll<HTMLElement>('[data-aging-group]')].find(n=>n.dataset.agingGroup===row.key&&n.getClientRects().length>0);setAccountReturn({key:row.key,scroll:window.scrollY,top:node?.getBoundingClientRect().top??0,page,sort});}
 ...
 return <section className="current-aging" aria-labelledby="current-aging-title">
  ...
  {selected&&selectedBucket&&<InvoiceDrill sortKey={invoiceSort} setSortKey={setInvoiceSort} search={invoiceSearch} setSearch={setInvoiceSearch} key={selected.key} externalRevision={revision} token={token} row={selected} bucket={selectedBucket} columns={columns} hotel={invoiceHotel} setHotel={setInvoiceHotel} setBucket={selectRange} refresh={refresh} onOpenInvoice={onOpenInvoice} view={invoiceView} setView={setInvoiceView} page={invoicePage} setPage={setInvoicePage} descending={invoiceDescending} setDescending={setInvoiceDescending}/>}
...
function InvoiceDrill({sortKey,setSortKey,search,setSearch,externalRevision,token,row,bucket,columns,hotel,setHotel,setBucket,refresh,onOpenInvoice,view,setView,page,setPage,descending,setDescending}:{sortKey:InvoiceSortKey;setSortKey:(v:InvoiceSortKey)=>void;search:string;setSearch:(v:string)=>void;externalRevision:number;token:string;row:AgingComparisonRow;bucket:AgingBucket;columns:AgingBucket[];hotel:AgingHotel;setHotel:(h:AgingHotel)=>void;setBucket:(key:string)=>void;refresh?:RefreshState|null;onOpenInvoice:Props['onOpenInvoice'];view:'all'|'bucket'|'unassigned';setView:(v:'all'|'bucket'|'unassigned')=>void;page:number;setPage:(page:number)=>void;descending:boolean;setDescending:(value:boolean)=>void}){
 ...
 const invoiceRows=filterSortInvoices((view==='all'?request.rows.filter(i=>i.verified&&i.open!==null&&i.open!==0&&['parent','standalone'].includes(i.role)&&!i.parentId):evidence.flatMap(e=>view==='unassigned'?e.unassigned:e.rows)).map(i=>({...i,status:invoiceProgress(i,thaiToday(),policy)})),search,sortKey,descending);
 const shown=invoiceRows;
 const reset=()=>setPage(1);
 const changeSort=(key:InvoiceSortKey)=>{setDescending(key===sortKey?!descending:key==='open');setSortKey(key);reset();};
 const credits=view==='unassigned'?[]:members.flatMap(a=>(a.agingAccountCredits??[]).filter(c=>view==='all'||c.bucketKey===agingBucketKey(bucket)).map(c=>({...c,hotel:a.hotel}))).filter(c=>!search.trim()||`${c.hotel} Account credit`.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase()));
 ...
 return <section className="aging-invoice-drill" aria-labelledby="aging-invoice-title">
  ...
  {state==='loading'?<p className="aging-empty" role="status">Loading current invoice evidence…</p>:state==='error'?<p className="aging-notice" role="alert">Invoice data is unavailable. No balance has been treated as zero. <button onClick={()=>setRevision(n=>n+1)}>Retry invoice read</button></p>:<>
   ...
   <div className="aging-table-scroll" tabIndex={0} role="region" aria-label="Current invoice evidence"><table className="aging-invoices" aria-label="Current aging invoices"><thead><tr>{invoiceColumns.map(([key,label])=><th key={key} scope="col" aria-sort={sortKey===key?(descending?'descending':'ascending'):'none'}><button aria-label={`Sort invoices by ${label}`} onClick={()=>changeSort(key)}>{label}{sortKey===key?(descending?' ↓':' ↑'):''}</button></th>)}</tr></thead><tbody>{shown.map(i=><tr key={JSON.stringify([i.hotel,i.accountId,i.id])}><td>{i.hotel}</td><td><button className="aging-invoice-link" aria-label={`Open invoice ${i.invoiceNo||i.id}`} onClick={()=>onOpenInvoice(i.hotel,i.accountId,i.id)}>{i.invoiceNo||'No invoice number'} <ArrowUpRight size={13}/></button></td><td>{i.folioNo||'—'}</td><td>{i.guest||'—'}</td><td>{i.date||'—'}</td><td>{i.age===null?'Unknown':i.age}</td><td className="aging-invoice-status" data-attention={i.status?.includes('Past due')||i.status?.includes('Urgent')||undefined}>{i.status}</td><td>{amount(i.open)}</td></tr>)}</tbody>{credits.length>0&&<tfoot>{credits.map((c,index)=><tr key={c.hotel+c.bucketKey+index}><td>{c.hotel}</td><td colSpan={6}>Account credit</td><td>{amount(-c.amount)}</td></tr>)}</tfoot>}</table>{shown.length===0&&<p className="aging-empty">{search.trim()?'No invoices match your search.':view==='unassigned'?'No unassigned invoices in this account scope.':'No invoices in this selection.'}</p>}</div>
   <div className="aging-invoice-count">{invoiceRows.length.toLocaleString()} invoices</div>
  </>}
 </section>;
}

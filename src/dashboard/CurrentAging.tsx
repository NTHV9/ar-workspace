...
import {ArrowLeft,ArrowUpRight,ChevronRight,RefreshCw} from 'lucide-react';
import {sourceAging,type Account,type AgingBucket,type RefreshState} from '../domain/portfolio';
import {agingBucketKey,agingColumns,agingComparison,agingHotels,agingInvoiceEvidence,agingPercentage,parseAgingInvoices,type AgingCell,type AgingComparisonRow,type AgingHotel,type AgingInvoice} from './aging-model';
import './aging.css';

export interface AgingContext {hotel:string;type?:string;accountKey?:string;search:string;page:number;sort:{key:string;hotel:AgingHotel;descending:boolean};bucketKey:string;invoiceHotel:AgingHotel;invoiceView:'bucket'|'unassigned';invoicePage:number;invoiceDescending:boolean}
...
function AgingWorkspace({revision=0,token,hotel,accounts,refresh,onOpenInvoice,initialContext,onContextChange}:Props){
 ...
 const context=useMemo<AgingContext>(()=>({hotel,type,accountKey,search,page,sort,bucketKey,invoiceHotel,invoiceView,invoicePage,invoiceDescending}),[hotel,type,accountKey,search,page,sort,bucketKey,invoiceHotel,invoiceView,invoicePage,invoiceDescending]);
 ...
 return <section className="current-aging" aria-labelledby="current-aging-title">
  ...
  <p className="aging-explanation">Amounts include credits. Each % is the bucket amount divided by this row’s net open for the same hotel, or Total. A zero or negative denominator has no %. Credits can produce negative shares or shares above 100%.</p>
  {columns.length?<div className="aging-table-scroll" tabIndex={0} role="region" aria-label="Scroll all source aging buckets"><table className="aging-matrix" aria-label="Current source aging comparison">
   <thead><tr><th rowSpan={2} scope="col" className="aging-identity" aria-sort={sortState('name')}><button onClick={()=>changeSort('name')}>{type===undefined?'Account Type':'Matched Account'}{sort.key==='name'?(sort.descending?' ↓':' ↑'):''}</button></th><th colSpan={3} scope="colgroup">Net open · THB</th>{columns.map(b=><th key={agingBucketKey(b)} colSpan={3} scope="colgroup">{b.label} days</th>)}</tr><tr>{['net',...columns.map(agingBucketKey)].flatMap(key=>agingHotels.map(h=><th key={key+h} scope="col" className={'aging-'+h.toLowerCase()} aria-sort={sortState(key,h)}><button aria-label={`Sort ${key==='net'?'net open':columns.find(b=>agingBucketKey(b)===key)?.label} ${h}`} onClick={()=>changeSort(key,h)}>{h}{sort.key===key&&sort.hotel===h?(sort.descending?' ↓':' ↑'):''}</button></th>))}</tr></thead>
   <tbody>{shown.map(row=><tr key={row.key}><th scope="row" className="aging-identity"><button className="aging-row-link" aria-label={type===undefined?`Open accounts in ${row.name}`:`Open invoices for ${row.name}`} onClick={()=>drill(row,invoiceHotel,selectedBucket)}>{row.name}<ChevronRight size={13}/></button><small>{type===undefined?`${row.members.length} hotel accounts`:row.members.map(a=>`${a.hotel} · ${a.account_no||a.id}`).join(' / ')}</small></th>{agingHotels.map(h=><td key={'net'+h} className={'aging-'+h.toLowerCase()}><strong>{amount(row.net[h].amount)}</strong>{row.net[h].state!=='verified'&&<small>{stateLabel[row.net[h].state]}</small>}</td>)}{columns.flatMap((bucket,index)=>agingHotels.map(h=>{
    const cell=row.cells[index][h],percentage=agingPercentage(cell.amount,row.net[h].amount);
    return <td key={agingBucketKey(bucket)+h} className={'aging-'+h.toLowerCase()}><button className="aging-cell-button" aria-label={`${row.name} · ${h} · ${bucket.label}`} disabled={cell.state==='absent'||cell.state==='outside'} onClick={()=>drill(row,h,bucket)}><strong>{amount(cell.amount)}</strong><small>{cell.state==='verified'?percentage===null?'% unavailable':`${percentage.toFixed(1)}%`:stateLabel[cell.state]}</small></button></td>;
   }))}</tr>)}</tbody>
  ...
  <p className="aging-footnote">Scroll horizontally for all {columns.length} source buckets. “No matching account” means no ledger in that hotel; it is different from a verified 0.00. Total includes the matching ledgers shown in the current hotel scope.</p>

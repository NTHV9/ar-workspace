import {isHotelId,hotelInRegion,type RegionId} from '../domain/hotels';
...
export const currentResult=(value:unknown)=>{const r=checkedSummary<CurrentSummary>(value);if(!Array.isArray(r.summary.hotels)||count(r.summary.invoices)===null||r.summary.hotels.some(h=>!object(h)||!isHotelId(h.hotel)||count(h.accounts)===null||count(h.unverified_accounts)===null||decimal(h.open)===null||decimal(h.over90)===null||h.oldest_sync!==null&&(typeof h.oldest_sync!=='string'||!Number.isFinite(Date.parse(h.oldest_sync)))))throw Error('dashboard_response_invalid');return r;};
...
export async function readDashboardOptions(token:string,signal:AbortSignal,region:RegionId='phuket'):Promise<AccountOption[]>{
 const all:AccountOption[]=[],seen=new Set<string>();let total:number|undefined;
 for(let page=0;;page++){
  const v=await read('/api/reports/options?limit=200&page='+page+(region==='khao-lak'?'&region=khao-lak':''),token,signal);

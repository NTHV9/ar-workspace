...
export function PeriodActivity({scope,token,revision,onReload,onDetail,overview}:{overview?:Source<DashboardHotelOverviewResponse>;scope:DashboardScope;token:string;revision:number;onReload:()=>void;onDetail:(detail:PeriodDetail)=>void}){
 ...
 return <section className="dashboard-period-activity" aria-label="Activity in selected period"><header className="dashboard-section-heading"><div><h2>Activity in selected period</h2><p>{rangeLabel(scope.from,scope.to)} · Calendar dates in Thailand</p></div><button disabled={busy||!valid||!status?.enabled||active.length>0||waitingForRefresh&&!!status?.running} onClick={()=>void refresh()}>{busy?'Requesting…':active.length===hotels.length?'OPERA refresh in progress':'Refresh OPERA for this period'}</button></header>
  ...
  <div className="dashboard-surface dashboard-activity-table"><header className="period-table-heading"><h3>Billing &amp; Follow-Up sent</h3><p>Activity recorded within this period</p></header><div className="dashboard-table-scroll" tabIndex={0} role="region" aria-label="Period invoice and billing activity"><table><thead><tr><th>Activity</th><th>{overview?'All hotels':'Invoices / occurrences'}</th>{overview&&<>{regionHotels(scope.region??'phuket').map(h=><th key={h}>{h}</th>)}</>}</tr></thead><tbody>{sendRows.map(r=><tr key={r.key}><th scope="row">{r.href?<a href={r.href}>{r.label}</a>:<button className="dashboard-text-link" onClick={()=>onDetail(r.detail!)}>{r.label}</button>}<small>{r.basis}</small></th><td className="period-activity-value" data-label={overview?'All hotels':accountIdentity(scope.account)?.[0]??scope.hotel}><strong><span data-testid={'dashboard-'+r.key+'-count'}>{number(r.count)}</span> <small>{r.unit}</small></strong><b>{amount(r.amount)}</b></td>{overview&&hotelMeasures(r.measureKey,r.detail).map(h=><td key={h.hotel} data-label={h.hotel}><HotelSplit rows={[h]} id={r.id} label={r.label.replace(/ · sent$/,'')} unit={r.unit}/></td>)}</tr>)}</tbody></table></div></div>

...
export function PeriodActivity({scope,token,revision,onReload,onDetail,overview}:{overview?:Source<DashboardHotelOverviewResponse>;scope:DashboardScope;token:string;revision:number;onReload:()=>void;onDetail:(detail:PeriodDetail)=>void}){
 ...
 const localEntries=useSource(path('/api/dashboard/invoice-entries'),token,revision,financialResult,true),localPayments=useSource(path('/api/financial/payments'),token,revision,financialResult,true),localPaid=useSource(path('/api/dashboard/payment-invoices'),token,revision,paidInvoicesResult,true);
 ...
 const activitySources=[activity,external,entries,payments,paid];
 const loading=activitySources.some(s=>s.state==='loading');
 const incomplete=[entries,payments].some(s=>s.state==='ready'&&!financialReady(s))||paid.state==='ready'&&!paid.data?.complete;
 const highlightState=(key:string)=>key==='invoice_entries'?entries.state:key==='paid'?paid.state:activity.state;
 ...
 const identity=accountIdentity(scope.account),hotels=identity?[identity[0]]:scope.hotel==='All'?regionHotels(scope.region??'phuket'):[scope.hotel];
 ...
 const statusUrl='/api/financial/status'+(scope.region==='khao-lak'?'?region=khao-lak':'');
 ...
 },[revision,statusUrl,scope.region,scope.hotel,scope.account,scope.from,scope.to]);
 ...
 const rows=[{key:'invoice_entries',label:'New invoices',count:financialReady(entries)?entries.data?.summary.invoiceCount:null,amount:financialReady(entries)?entries.data?.summary.amount:null,basis:'OPERA Bill Date · Original invoice value',detail:{kind:'invoice_entries'} as PeriodDetail},
 ...
 const hotelMeasures=(key:string,detail?:PeriodDetail):HotelMeasure[]=>overview?regionHotels(scope.region??'phuket').map(hotel=>{
 ...
 const paymentMeasures=(key:'creditPostings'|'currentlyApplied'|'currentlyUnallocated'|'debitPostings'):HotelMeasure[]=>overview?regionHotels(scope.region??'phuket').map(hotel=>{const p=overview.data?.hotels.find(h=>h.hotel===hotel)?.payments;return {hotel,amount:p?.coverage.complete&&financialMembershipKnown(p.summary)?p.summary.paymentTotals?.[key]??null:null,onOpen:()=>onDetail({kind:'payments',hotel})};}):[];

 const paymentColumns:MatrixColumn[]=(['creditPostings','currentlyApplied','currentlyUnallocated','debitPostings'] as const).map((key,index)=>({key,label:['Recorded payment credits','Currently allocated','Unallocated credit','Debit postings / corrections'][index],id:'payment-'+key,measures:paymentMeasures(key),total:{amount:knownPayments?.[key],amountId:key==='creditPostings'?'dashboard-payment-credits':undefined,onOpen:()=>onDetail({kind:'payments'})}}));
 const sendRows=[
 ...
 return <section className="dashboard-period-activity" aria-label="Activity in selected period"><header className="dashboard-section-heading"><div><h2>Activity in selected period</h2><p>{rangeLabel(scope.from,scope.to)} · Calendar dates in Thailand</p></div><button disabled={busy||!valid||!status?.enabled||active.length>0||waitingForRefresh&&!!status?.running} onClick={()=>void refresh()}>{busy?'Requesting…':active.length===hotels.length?'OPERA refresh in progress':'Refresh OPERA for this period'}</button></header>
  ...
  {loading&&<p className="dashboard-refresh-note" role="status">Loading activity… Available results are shown as they arrive.</p>}
  {incomplete&&<p className="dashboard-footnote">Some OPERA totals are not verified for this period. Available detail rows remain inspectable; missing totals display —.{paid.data?.unknownMappings?' '+paid.data.unknownMappings+' payment mappings need verification.':''}</p>}
  {!loading&&overview?.data?.hotels.some(h=>[h.activity,h.external,h.entries,h.payments,h.paid].some(p=>p===null))&&<p className="dashboard-notice" role="status">Some hotel activity breakdowns are unavailable. Other verified results remain visible; reload to retry.</p>}
  ...
  <div className="dashboard-activity-highlights">{rows.filter(r=>['invoice_entries','first','paid'].includes(r.key)).map(r=><article key={r.key} data-activity={r.key}><button onClick={()=>onDetail(r.detail)}><span className="period-activity-name"><span className="period-activity-symbol" aria-hidden="true">{r.key==='invoice_entries'?<FilePlus2 size={20}/>:r.key==='first'?<MailCheck size={20}/>:<Wallet size={20}/>}</span>{r.label}<ArrowUpRight size={14} aria-hidden="true"/></span><div><strong data-testid={'dashboard-'+r.key+'-count'}>{number(r.count)}</strong><span>invoices</span><b>{amount(r.amount)}</b></div><small>{highlightState(r.key)==='loading'?'Loading…':highlightState(r.key)==='error'?'Could not load · Reload dashboard':r.count==null?'Data incomplete · View available records':r.basis}</small></button></article>)}</div>
  {overview&&<PeriodHotelMatrix title="Activity by hotel" description="Invoices and amounts recorded within the selected period · THB" hotels={regionHotels(scope.region??'phuket')} columns={rows.filter(r=>['invoice_entries','first','paid'].includes(r.key)).map(r=>({key:r.key,label:r.label,id:'activity-'+r.key,measures:hotelMeasures(r.key,r.detail)}))}/>}
  <div className="dashboard-surface dashboard-payment-summary"><header><h3>OPERA payments dated in this period</h3><button className="dashboard-text-link" onClick={()=>onDetail({kind:'payments'})}>View payment records</button></header>{overview?<PeriodHotelMatrix title="Payment amounts by hotel" description="Current allocation of payments dated in this period · THB" hotels={regionHotels(scope.region??'phuket')} columns={paymentColumns} showTotals className="embedded"/>:<dl><div><dt>Recorded payment credits</dt><dd data-testid="dashboard-payment-credits">{amount(knownPayments?.creditPostings)}</dd><HotelSplit rows={paymentMeasures('creditPostings')} id="payment-creditPostings" label="Payment creditPostings"/></div><div><dt>Currently allocated</dt><dd>{amount(knownPayments?.currentlyApplied)}</dd><HotelSplit rows={paymentMeasures('currentlyApplied')} id="payment-currentlyApplied" label="Payment currentlyApplied"/></div><div><dt>Unallocated credit</dt><dd>{amount(knownPayments?.currentlyUnallocated)}</dd><HotelSplit rows={paymentMeasures('currentlyUnallocated')} id="payment-currentlyUnallocated" label="Payment currentlyUnallocated"/></div><div><dt>Debit postings / corrections</dt><dd>{amount(knownPayments?.debitPostings)}</dd><HotelSplit rows={paymentMeasures('debitPostings')} id="payment-debitPostings" label="Payment debitPostings"/></div></dl>}<p className="dashboard-footnote">Partial payments count when their invoice links are verified. These are current allocations of payments dated in the period; OPERA does not supply the allocation event date. Source {stamp(payments.data?.coverage.lastSuccessAt)}.</p></div>
  <div className="dashboard-surface dashboard-activity-table"><header className="period-table-heading"><h3>Billing &amp; Follow-Up sent</h3><p>Activity recorded within this period</p></header><div className="dashboard-table-scroll" tabIndex={0} role="region" aria-label="Period invoice and billing activity"><table><thead><tr><th>Activity</th><th>{overview?'All hotels':'Invoices / occurrences'}</th>{overview&&<>{regionHotels(scope.region??'phuket').map(h=><th key={h} data-hotel={h}>{h}</th>)}</>}</tr></thead><tbody>{sendRows.map(r=><tr key={r.key}><th scope="row">{r.href?<a href={r.href}>{r.label}</a>:<button className="dashboard-text-link" onClick={()=>onDetail(r.detail!)}>{r.label}</button>}<small>{r.basis}</small></th><td className="period-activity-value" data-label={overview?'All hotels':accountIdentity(scope.account)?.[0]??scope.hotel}><strong><span data-testid={'dashboard-'+r.key+'-count'}>{number(r.count)}</span> <small>{r.unit}</small></strong><b>{amount(r.amount)}</b></td>{overview&&hotelMeasures(r.measureKey,r.detail).map(h=><td key={h.hotel} data-label={h.hotel} data-hotel={h.hotel}><HotelSplit rows={[h]} id={r.id} label={r.label.replace(/ · sent$/,'')} unit={r.unit}/></td>)}</tr>)}</tbody></table></div></div>

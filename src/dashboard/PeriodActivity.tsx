...
export function PeriodActivity({scope,token,revision,onReload,onDetail}:{scope:DashboardScope;token:string;revision:number;onReload:()=>void;onDetail:(detail:PeriodDetail)=>void}){
 ...
 const activity=useSource(path('/api/reports/activity'),token,revision,activityResult,true),external=useSource(path('/api/external-billing'),token,revision,externalResult,true);
 const entries=useSource(path('/api/financial/invoice_entries'),token,revision,financialResult,true),payments=useSource(path('/api/financial/payments'),token,revision,financialResult,true),paid=useSource(path('/api/dashboard/payment-invoices'),token,revision,paidInvoicesResult,true);

...
export type InvoiceSortKey='hotel'|'invoiceNo'|'folioNo'|'guest'|'date'|'age'|'open'|'status';
export const invoiceColumns:readonly [InvoiceSortKey,string][]=[['hotel','Hotel'],['invoiceNo','Invoice No.'],['folioNo','Folio No.'],['guest','Guest'],['date','Source date'],['age','OPERA age'],['status','Status'],['open','Open · THB']];
...
export function filterSortInvoices(rows:AgingInvoice[],search:string,key:InvoiceSortKey,descending:boolean){
 ...
 return rows.filter(row=>!query||[row.hotel,row.invoiceNo,row.folioNo,row.guest,row.status??''].some(value=>text(value).includes(query))).sort((a,b)=>{
  const x=a[key],y=b[key],empty=(v:string|number|null|undefined)=>v==null||v==='';

...
export default function Dashboard({token,hotel,accounts,params,update,refresh,onOpenInvoice,onReloadCatalog,initialAgingContext,onAgingContextChange}:{token:string;hotel:string;accounts:Account[];params:URLSearchParams;update:(fields:Record<string,string>)=>void;refresh:RefreshState|null|undefined;onOpenInvoice:(hotel:string,accountId:string,invoiceId?:string)=>void;onReloadCatalog?:()=>void;initialAgingContext?:AgingContext;onAgingContextChange?:(context:AgingContext)=>void}){
 ...
 const overview=useSource(comparing&&view==='period'&&validPeriod(scope,today)?'/api/dashboard/hotel-overview?'+overviewQuery:null,token,revision,value=>hotelOverviewResult(value,scope.from,scope.to,region),true,mergeHotelOverview);
 ...
 return <main className={'page dashboard-page'+(view==='period'?' period-analysis'+(comparing?' period-comparison':''):'')}><header className="dashboard-title"><div><h1>Dashboard</h1><p>Receivables, billing and collections at a glance.</p></div><nav className="dashboard-view-tabs" aria-label="Dashboard views"><button aria-pressed={view==='period'} onClick={()=>update({dashboardView:'period'})}>Period analysis</button><button aria-pressed={view==='aging'} onClick={()=>update({dashboardView:'aging'})}>Current Aging · {region==='phuket'?hotels.join(' / '):'Khao Lak'}</button></nav><button onClick={reload}><RefreshCw size={14}/> Reload dashboard</button></header>

  {view==='aging'?<CurrentAging region={region} revision={revision+Date.parse(today)} token={token} hotel={hotel} accounts={accounts} refresh={refresh} onOpenInvoice={onOpenInvoice} initialContext={initialAgingContext} onContextChange={onAgingContextChange}/>:<>

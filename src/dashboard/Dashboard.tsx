...
import {readDashboardOptions,useSource} from './data';
import {hotelOverviewResult,mergeHotelOverview} from './hotel-data';
...
export default function Dashboard({token,hotel,accounts,params,update,refresh,onOpenInvoice,onReloadCatalog,initialAgingContext,onAgingContextChange}:{token:string;hotel:string;accounts:Account[];params:URLSearchParams;update:(fields:Record<string,string>)=>void;refresh:RefreshState|null|undefined;onOpenInvoice:(hotel:string,accountId:string,invoiceId?:string)=>void;onReloadCatalog?:()=>void;initialAgingContext?:AgingContext;onAgingContextChange?:(context:AgingContext)=>void}){
 ...
 const overviewQuery=new URLSearchParams({from:scope.from,to:scope.to});if(scope.type)overviewQuery.set('type',scope.type);
 const comparing=hotel==='All'&&!accountIdentity(scope.account);
 const overview=useSource(comparing&&view==='period'&&validPeriod(scope,today)?'/api/dashboard/hotel-overview?'+overviewQuery:null,token,revision,value=>hotelOverviewResult(value,scope.from,scope.to),true,mergeHotelOverview);
 const detailHotel=hotel==='All'&&!scope.account&&['KAT','TSK'].includes(params.get('dashboardDetailHotel')??'')?params.get('dashboardDetailHotel'):null;
 ...
 const change=(fields:Record<string,string>)=>update({...fields,dashboardDetail:'',dashboardStage:'',dashboardMetric:'',dashboardPage:'',dashboardDetailHotel:''});
 const openDetail=(d:PeriodDetail)=>{detailScroll.current=window.scrollY;update({dashboardDetail:d.kind,dashboardMetric:d.metric??'',dashboardStage:d.stage??'',dashboardPage:'0',dashboardDetailHotel:d.hotel??''});};
 ...
 return <main className={'page dashboard-page'+(view==='period'?' period-analysis':'')}><header className="dashboard-title"><div><h1>Dashboard</h1><p>Receivables, billing and collections at a glance.</p></div><nav className="dashboard-view-tabs" aria-label="Dashboard views"><button aria-pressed={view==='period'} onClick={()=>update({dashboardView:'period'})}>Period analysis</button><button aria-pressed={view==='aging'} onClick={()=>update({dashboardView:'aging'})}>Current Aging · KAT / TSK</button></nav><button onClick={reload}><RefreshCw size={14}/> Reload dashboard</button></header>

  {view==='aging'?<CurrentAging revision={revision+Date.parse(today)} token={token} hotel={hotel} accounts={accounts} refresh={refresh} onOpenInvoice={onOpenInvoice} initialContext={initialAgingContext} onContextChange={onAgingContextChange}/>:<>
   ...
   {!datesValid?<p className="dashboard-notice" role="alert">Choose valid dates, with From on or before Through and no future dates (maximum 10 years).</p>:<div key={JSON.stringify(scope)}><PeriodBalances overview={comparing?overview:undefined} scope={scope} token={token} revision={revision} onDetail={openDetail}><PeriodActivity overview={comparing?overview:undefined} scope={scope} token={token} revision={revision} onReload={reload} onDetail={openDetail}/></PeriodBalances>{detail&&<PeriodDetailPanel scope={detailScope} detail={detail} page={detailPage} token={token} revision={revision} onPage={page=>update({dashboardPage:String(page)})} onClose={()=>change({})} onOpenInvoice={onOpenInvoice}/>}</div>}
  </>}
 </main>;
}

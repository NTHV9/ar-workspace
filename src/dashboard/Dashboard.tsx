...
function DashboardContent({scope,token,revision,today,refresh}:{scope:DashboardScope;token:string;revision:number;today:string;refresh:RefreshState|null|undefined}){
 ...
 return <>
  ...
  <div className="dashboard-evidence-grid">
   ...
   <section className="dashboard-surface dashboard-payment-detail"><h2>How payment credits are allocated</h2><p>Current allocation of payments dated {dateValid?dayLabel(scope.day):'on the selected day'}.</p><dl><div><dt>Applied to invoices</dt><dd>{amount(payment?.currentlyApplied)}</dd></div><div><dt>Unallocated credit</dt><dd>{amount(payment?.currentlyUnallocated)}</dd></div><div><dt>Debit payment postings</dt><dd>{amount(payment?.debitPostings)}</dd></div></dl><small>Allocation has no supplied event date. Debit postings are not labelled as cancellations without evidence.{payment&&payment.transferRows>0?` ${payment.transferRows} transfer-marked records are included; inspect their source details.`:''}</small></section>
  </div>
  <div className="dashboard-evidence-grid">

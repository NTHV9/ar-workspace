...
/** Expand only unchanged legacy default labels; captured custom wording stays exact. */
export function displayCollectionStageLabel(key:string,label?:string|null){const value=label??key;return value===key&&/^Follow [123]$/.test(key)?key.replace('Follow ','Follow-up '):value;}
export function policyStageLabel(key:string,policy?:CollectionPolicy|null,snapshot?:StageSnapshot|null){return displayCollectionStageLabel(key,snapshot?.key===key?snapshot.label:policy?.rounds.find(r=>r.key===key)?.label??legacyStageSnapshot(key)?.label);}

...
export function normalizeReceived(value:string):string|null{const v=value.trim();if(!v)return null;if(!/^(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d{1,2})?$/.test(v))throw Error('Use a non-negative amount with no more than two decimal places.');const amount=v.replaceAll(',','');if(!Number.isSafeInteger(Math.round(Number(amount)*100)))throw Error('The amount is too large.');return amount;}
function valueFor(field:keyof RegisterValues,value:string,today:string,stages:{key:string;label:string}[]):RegisterValues[keyof RegisterValues]{
 ...
 if(field==='reportedReceived')return normalizeReceived(value);

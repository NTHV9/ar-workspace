...
export function PrivatePdfPreview({jobId,fileId,token,maxBytes,onClose}:{jobId:string;fileId:string;token:string;maxBytes:number;onClose:()=>void}){
 ...
 useEffect(()=>{const element=dialog.current;if(!element)return;return observeDisplay(element,()=>setDisplay(element.clientWidth*devicePixelRatio));},[]);
 ...
 useEffect(()=>{if(!pdf)return;let active=true;let task:RenderTask|undefined;setRendering(true);setError('');
  void(async()=>{try{const source=await pdf.getPage(page);if(!active||!canvas.current)return;const original=source.getViewport({scale:1});const scale=Math.min(displayScale(original,canvas.current.getBoundingClientRect().width,devicePixelRatio),Math.sqrt(8_000_000/(original.width*original.height)));const viewport=source.getViewport({scale});canvas.current.width=Math.ceil(viewport.width);canvas.current.height=Math.ceil(viewport.height);task=source.render({canvas:canvas.current,viewport});await task.promise;}catch{if(active)setError('This page could not be rendered. Choose another page or retry the document.');}finally{if(active)setRendering(false);}})();return()=>{active=false;task?.cancel();};
 },[pdf,page,display]);

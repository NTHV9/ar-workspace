...
function PageCanvas({ page, documents, className = '', scale = 1.5 }: { page: PdfProjectPage; documents: Map<string, PDFDocumentProxy>; className?: string; scale?: number }) {
  ...
  useEffect(() => {
    ...
    renderPage(page, documents, canvas, rasterScale).then(() => { if (current) { container.current?.replaceChildren(canvas); setError(''); } else canvas.width=canvas.height=0; }).catch(() => { if (current) setError('Page could not be rendered. Reopen the original document.'); });
...
export function PdfWorkspace({ documents: sources, initialProject, initialDelivery='combined', initialReviewed=false,onContinueToEmail, accountName, hotel, selectedCount, onClose, onSave, onSaveDraft }: Props) {
  ...
  return <div className="pdf-workspace" data-companion={companion} ref={workspace} role="dialog" aria-modal={!preview&&!closeConfirm} aria-label="PDF Workspace" onKeyDown={event => {
    ...
    <nav className="pdf-steps" aria-label="Document steps">{['Scope & Purpose', 'Documents', 'PDF Review', 'Email', 'Handoff'].map((name, i) => <div key={name} className={i === 2 ? 'current' : i < 2 ? 'done' : ''}><span>{i < 2 ? <Check size={15}/> : i + 1}</span><div><b>{name}</b><small>{['Hotel & Account', 'Selected source PDFs', 'Edit, preview, flatten', onContinueToEmail?'Prepare after saving':'Separate email workspace', 'Private save / download'][i]}</small></div></div>)}</nav>

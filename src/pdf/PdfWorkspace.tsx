...
import {invoiceTargets,validateInvoiceAttachments,moveInvoiceAttachment,removeInvoiceAttachment,MAX_INVOICE_PDFS,MAX_ATTACHED_PAGES} from './invoice-attachments';
import {inspectSupplemental} from '../../worker/email/supplemental-validation';
import {linkedVoucher,syncVouchers} from './linked-vouchers';
import {voucherBindings} from './engine';
...
export function PdfWorkspace({ maxBytes=67108864,onDirtyChange, transient=false, onReview, documents: sources, initialProject, initialDelivery='combined', initialReviewed=false,onContinueToEmail, accountName, hotel, selectedCount, onClose, onSave, onSaveDraft }: Props) {
  ...
  const [attachedSources,setAttachedSources]=useState<PdfSourceDocument[]>([]);
  const allSources=[...sources,...attachedSources];
  const attachmentDisposers=useRef<(()=>void)[]>([]),attachmentLoading=useRef(false);
  ...
  const [viewed, setViewed] = useState<number[]>([]),[previewRendering,setPreviewRendering]=useState(true);
  useEffect(()=>{if(companion){setPreview(null);}},[companion]);
  ...
  useEffect(() => {
    ...
    revision.current++; reviewedProject.current = null;setAttachedSources([]);attachmentDisposers.current=[];
    setProject(null); setPersistedProject(null); setDraftSaved(false); setCloseConfirm(false); setError(''); setPreview(null); setViewed([]); setSaved(false); setSelected('');
    loadSources(sources).then(result => { dispose = result.dispose; if (current) { sourceModel.current=result.project; const restored = initialProject ? restoreProject(initialProject, result.project,sources) : {...result.project,delivery:initialDelivery}; setLoaded(result.documents); setProject(restored); setPersistedProject(restored); setActiveId(restored.pages[0]?.id || ''); setPast([]); setFuture([]); } else dispose(); }).catch(e => { dispose?.(); if (current) setError(e instanceof Error ? e.message : 'Unable to open PDF.'); });
    return () => { current = false; revision.current++; dispose?.();for(const release of attachmentDisposers.current)release();attachmentDisposers.current=[]; };
  }, [sources, initialProject, initialDelivery]);
  const visiblePages = project ? deliveryGroups(project, allSources).flatMap(g => g.pages) : [];
  ...
  function invalidate() { revision.current++; reviewedProject.current = null; setPreview(null); setViewed([]); setSaved(false); setDraftSaved(false); }
  ...
  function commit(next: PdfProject,catalog:PdfSourceDocument[]=allSources) { if (!project||!sourceModel.current) return; try{next=syncVouchers(project,next,voucherBindings(loaded));}catch(e){setError(e instanceof Error?e.message:'This edit could not be applied.');return;}try{restoreProject(next,sourceModel.current,catalog);validateInvoiceAttachments(next,catalog);}catch{setError('This change exceeds safe page, layer, text or image limits and was not applied.');return;} setError('');setPast(p => [...p.slice(-49), project]); setFuture([]); setProject(next); invalidate();return true; }
  ...
  function updateLayer(change:Partial<PdfLayer>){
    ...
    if('text'in change&&linkedVoucher(page,layer,voucherBindings(loaded))){commit({...project,pages:project.pages.map(p=>p.id===page.id?{...p,layers:p.layers.map(l=>l.id===layer.id?{...l,text:change.text!}:l)}:p)});return;}
    if(textChange){
      next=fitText(next);
  ...
  async function makePreview() {
    ...
    try {
      const files=await exportProject(project,allSources,loaded);
      if(startedAt!==revision.current||session!==sourceSession.current)return;
      reviewedProject.current=project;setPreview(files);setPreviewIndex(0);setViewed([]);
    }catch(e){
  ...
  async function addInvoicePdfs(files:File[],invoiceId:string){
    if(!project||busy||attachmentLoading.current||!sourceModel.current)return;
    if(!invoiceTargets(sources).some(t=>t.id===invoiceId)){setError('Choose an invoice in this preparation.');return;}
    if(attachedSources.length+files.length>MAX_INVOICE_PDFS){setError('This tab retains up to 50 imported PDFs, including Undo history. Start a new preparation for another set.');return;}
    if(files.some(f=>!f.size||! /\.pdf$/i.test(f.name)||f.name.length>200)){setError('Choose non-empty PDF files with filenames up to 200 characters.');return;}
    if([...sources,...attachedSources].reduce((n,s)=>n+s.bytes.length,0)+files.reduce((n,f)=>n+f.size,0)>maxBytes){setError('These PDFs exceed this tab’s memory limit, including Undo history. Choose smaller files or start a new preparation.');return;}
    const session=sources,startedAt=revision.current;attachmentLoading.current=true;setBusy(true);setError('');let additions:Awaited<ReturnType<typeof loadSources>>|undefined;
    try{
      const nextSources:PdfSourceDocument[]=[];let pages=sourceModel.current.pages.filter(p=>attachedSources.some(s=>s.id===p.sourceId)).length;
      for(const file of files){const bytes=new Uint8Array(await file.arrayBuffer());const inspection=await inspectSupplemental(bytes,file.name,'application/pdf');pages+=inspection.pages??0;if(pages>MAX_ATTACHED_PAGES)throw Error('Attached PDFs exceed 500 pages. Choose a smaller set of files.');nextSources.push({id:'attached-'+uid(),name:file.name,kind:'attachment',bytes});}
      additions=await loadSources(nextSources);
      if(startedAt!==revision.current||session!==sourceSession.current){additions.dispose();additions=undefined;return;}
      const catalog=[...allSources,...nextSources],base=sourceModel.current;
      const next:PdfProject={...project,content:project.content==='statement'?'both':project.content,pages:[...project.pages,...additions.project.pages],invoiceAttachments:[...(project.invoiceAttachments??[]),...nextSources.map(s=>({sourceId:s.id,invoiceId}))]};
      sourceModel.current={...base,pages:[...base.pages,...additions.project.pages]};
      for(const [id,doc] of additions.documents)loaded.set(id,doc);
      if(!commit(next,catalog)){sourceModel.current=base;for(const s of nextSources)loaded.delete(s.id);additions.dispose();additions=undefined;return;}
      attachmentDisposers.current.push(additions.dispose);additions=undefined;setAttachedSources(p=>[...p,...nextSources]);setActiveId(nextSources[0].id+':1');setSelected('');
    }catch(e){additions?.dispose();if(startedAt===revision.current&&session===sourceSession.current){const code=e instanceof Error?e.message:'';setError(({attachment_active_pdf:'This PDF contains forms, scripts or embedded files. Print or save a static PDF copy, then add that copy.',attachment_invalid_pdf:'This PDF is damaged or password-protected. Choose an unlocked PDF.',attachment_invalid:'This file is not a valid PDF.',attachment_pdf_complexity:'This PDF is too complex to import. Try a simpler PDF copy.'} as Record<string,string>)[code]||code||'The PDFs could not be added.');}}
    finally{attachmentLoading.current=false;if(session===sourceSession.current)setBusy(false);}
  }
  async function save(next:'email'|'download'='email') { if (!project || reviewedProject.current !== project || !preview || busy || previewRendering) return; const startedAt=revision.current;const session=sources;setBusy(true); setError(''); try { let complete:void|(()=>void)=undefined; if (onReview) complete=await onReview(preview,next); else if (onSave) await onSave(preview, reviewedProject.current); else for (const file of preview) { const url = URL.createObjectURL(new Blob([file.bytes.slice().buffer as ArrayBuffer], { type: 'application/pdf' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = file.name; anchor.click(); setTimeout(() => URL.revokeObjectURL(url), 60000); } if(startedAt===revision.current&&session===sourceSession.current){setSaved(true);setPersistedProject(project);if(typeof complete==='function')complete();} } catch(e) { if(startedAt===revision.current&&session===sourceSession.current)setError(e instanceof Error ? e.message : 'The handoff could not be confirmed. Keep this tab open and retry with the same reviewed files.'); } finally { setBusy(false); } }
  const changeCount = (project?.invoiceAttachments?.length??0) + (project?.pages.reduce((n, p) => n + p.layers.length + (p.rowEdits?.length??0) + (p.sourcePage === null ? 1 : 0), 0) || 0);
  ...
  return <div className="pdf-workspace" data-companion={companion} data-guides={showGuides} data-package-open={packageOpen} ref={workspace} role="dialog" aria-modal={!preview&&!closeConfirm} aria-label="PDF Workspace" onKeyDown={event => {
    ...
    {!project ? <div className="pdf-empty">{error ? 'Document unavailable. Close and retry from the Account.' : 'Opening source PDFs…'}</div> : <div className="pdf-layout" inert={busy||!!preview||closeConfirm}>
      <aside className="pdf-package" id="pdf-package-panel" hidden={!packageOpen}><h2>Package</h2><h3>Document content</h3>{([['statement','Statement only'],['invoices','Invoices'],['both','Both']] as const).map(([value,label]) => <button key={value} className={`pdf-choice ${project.content === value ? 'chosen' : ''}`} disabled={value === 'statement' ? !sources.some(s=>s.kind==='statement') : value === 'invoices' ? !sources.some(s=>s.kind==='invoice') : !sources.some(s=>s.kind==='statement') || !sources.some(s=>s.kind==='invoice')} onClick={()=>commit({...project,content:value})}>{label}</button>)}
        ...
        {(transient||!onSaveDraft)&&<InvoiceAttachments project={project} sources={allSources} activeInvoice={(()=>{const source=allSources.find(s=>s.id===page?.sourceId);return source?.kind==='invoice'?(source.invoiceId||source.id):project.invoiceAttachments?.find(a=>a.sourceId===page?.sourceId)?.invoiceId;})()} onAdd={(files,id)=>void addInvoicePdfs(files,id)} onAssign={(id,invoiceId)=>commit({...project,invoiceAttachments:project.invoiceAttachments?.map(a=>a.sourceId===id?{...a,invoiceId}:a)})} onMove={(id,direction)=>commit(moveInvoiceAttachment(project,id,direction))} onRemove={id=>commit(removeInvoiceAttachment(project,id))}/>}

...
import {mapSourceRect,overlaps} from './row-layout';
...
function PageCanvas({ page, documents, className = '', scale = 1.5 }: { page: PdfProjectPage; documents: Map<string, PDFDocumentProxy>; className?: string; scale?: number }) {
  ...
  useEffect(() => {
    ...
    renderPage(page, documents, canvas, rasterScale).then(() => { if (current) { container.current?.replaceChildren(canvas); setRendered(page);setError(''); } else canvas.width=canvas.height=0; }).catch(error => { if (current) setError(error instanceof Error?error.message:'Page could not be rendered. Reopen the original document.'); });
    return () => { current = false; };
  }, [page, documents, rasterScale]);
  return <div className={`pdf-canvas ${className}`} ref={container} data-render-state={rendered===page?'ready':'rendering'} aria-label="PDF page">{error && <p role="alert">{error}</p>}</div>;
}

function FinalPdfPreview({ file, zoom, onRendered, onRendering }: { file: PdfExportFile; zoom:number; onRendered: () => void; onRendering: () => void }) {
  const container = useRef<HTMLDivElement>(null);
  ...
  const [error, setError] = useState('');
  useEffect(() => {
    let current = true; let dispose: (() => void) | undefined,stopDisplay:(()=>void)|undefined,visibility:IntersectionObserver|undefined;
    let rendering=false,pending=false,failed=false;const canvases:HTMLCanvasElement[]=[];
    ...
    void (async () => {
      try {
        ...
        const pages=result.project.pages,scales=new Map<HTMLCanvasElement,number>();
        for(const [index,page] of pages.entries()){
          const canvas=document.createElement('canvas');canvas.width=Math.ceil(page.width*.5);canvas.height=Math.ceil(page.height*.5);canvas.style.visibility='hidden';
          canvas.setAttribute('role','img');canvas.setAttribute('aria-label',`Final PDF page ${index+1}`);canvases.push(canvas);container.current?.appendChild(canvas);
        }
        const rerender=async()=>{
          if(failed)return;pending=true;if(rendering)return;rendering=true;
          try{
            while(pending&&current){pending=false;const bounds=container.current!.parentElement!.getBoundingClientRect();let started=false;
              for(const [index,page] of pages.entries()){
                if(!current)return;const canvas=canvases[index],rect=canvas.getBoundingClientRect();
                const visible=rect.bottom>bounds.top-200&&rect.top<bounds.bottom+200;
                // Keep offscreen pages lightweight; sharpen them when scrolled into view.
                const scale=visible?displayScale(page,rect.width,devicePixelRatio):Math.min(.5,Math.sqrt(16_000_000/(pages.length*page.width*page.height)));
                if(Math.abs((scales.get(canvas)??0)-scale)<.01)continue;
                if(!started){starting.current();setStatus('Rendering actual output pages…');started=true;}
                await renderPage(page,result.documents,canvas,scale);scales.set(canvas,scale);canvas.style.visibility='visible';
              }
            }
            if(current){setError('');setStatus(`${pages.length} output pages rendered. Review every page below.`);ready.current();}
          }catch{failed=true;if(current){starting.current();container.current?.replaceChildren();canvases.forEach(c=>{c.width=c.height=0;});setError('Final PDF could not be rendered. Close the preview and retry. Saving remains disabled.');setStatus('');}}
          finally{rendering=false;if(!current)canvases.forEach(c=>{c.width=c.height=0;});}
        };
        await rerender();if(!current||failed)return;
        visibility=new IntersectionObserver(()=>void rerender(),{root:container.current!.parentElement,rootMargin:'200px'});canvases.forEach(c=>visibility!.observe(c));
        stopDisplay=observeDisplay(container.current!,()=>void rerender());
      } catch { if (current) { setError('Final PDF could not be rendered. Close the preview and retry. Saving remains disabled.'); setStatus(''); } }
    })();
    return () => { current = false; visibility?.disconnect();stopDisplay?.();dispose?.();if(!rendering)canvases.forEach(c=>{c.width=c.height=0;}); };
  }, [file]);
  return <div className="pdf-final-scroll"><p role="status">{status}</p>{error && <p role="alert" className="pdf-error">{error}</p>}<div className="pdf-final-pages" style={{width:`${zoom}%`,maxWidth:745*zoom/100}} ref={container}/></div>;
}
...
export function PdfWorkspace({ onDirtyChange, transient=false, onReview, documents: sources, initialProject, initialDelivery='combined', initialReviewed=false,onContinueToEmail, accountName, hotel, selectedCount, onClose, onSave, onSaveDraft }: Props) {
  ...
  const textCollision=!!layer?.sourceText&&!!textMetrics&&runs.some(run=>{if(layer.maskOriginal!==false&&run.sourceText?.runIndex===layer.sourceText?.runIndex)return false;const rect=mapSourceRect(run,page?.rowEdits??[]);return !!rect&&overlaps({x:layer.x,y:layer.y,width:textMetrics.width,height:textMetrics.height},rect);});
  ...
  function fitText(next:PdfLayer):PdfLayer {
    ...
    const natural=measureLayerText(next);const width=Math.min(page.width-next.x,Math.max(next.original?.width??next.width,natural.width+(next.sourceText?1:0)));
    const measured=measureLayerText({...next,width});return {...next,width,height:Math.min(page.height-next.y,Math.max(next.original?.height??next.fontSize*1.25,measured.height))};
  }
  function updateLayer(change: Partial<PdfLayer>) { if (page && layer && project) {const merged={...layer,...change},next=('text'in change||'fontSize'in change||'font'in change||'bold'in change||'italic'in change)?fitText(merged):merged;commit({...project,pages:project.pages.map(p=>p.id===page.id?{...p,layers:p.layers.map(l=>l.id===layer.id?next:l)}:p)});} }
  function focusText(caret?:number){requestAnimationFrame(()=>{textField.current?.focus();if(caret!==undefined)textField.current?.setSelectionRange(caret,caret);});}
  function changeLine(remove=false){if(!layer)return;const input=textField.current,start=input?.selectionStart??layer.text.length,end=input?.selectionEnd??start;const edit=remove?removeTextLine(layer.text,start,end):insertTextLine(layer.text,start,end);updateLayer({text:edit.text});focusText(edit.caret);}
  function restoreFormatting(){if(!layer||!originalRun||!page)return;const fresh=createReplacementLayer(originalRun,layer.id),position=layer.maskOriginal===false?layer:mapSourceRect(originalRun,page.rowEdits??[]);if(!position)return;updateLayer({...fresh,x:position.x,y:position.y,text:layer.text,...(layer.maskOriginal===false?{maskOriginal:false}:{})});focusText();}
  ...
  function addLayer(kind: PdfLayer['kind'], original?: DetectedText, image?: string) {
    ...
    if(kind==='replacement'&&original){
      ...
      const position=mapSourceRect(original,page.rowEdits??[]);if(!position)return;const item=createReplacementLayer(original,uid());item.x=position.x;item.y=position.y;commit({...project,pages:project.pages.map(p=>p.id===page.id?{...p,layers:[...p.layers,item]}:p)});setSelected(item.id);setTool('replace');focusText();return;
    }
  ...
  return <div className="pdf-workspace" data-companion={companion} ref={workspace} role="dialog" aria-modal={!preview&&!closeConfirm} aria-label="PDF Workspace" onKeyDown={event => {
    ...
    {!project ? <div className="pdf-empty">{error ? 'Document unavailable. Close and retry from the Account.' : 'Opening source PDFs…'}</div> : <div className="pdf-layout" inert={busy}>
    ...
    {preview&&<div className="pdf-preview-overlay"><section className="pdf-preview-dialog" role="dialog" aria-modal="true" aria-label="Final PDF preview"><header><div><h2>Final PDF preview</h2><p>{preview[previewIndex].name} · {previewIndex+1} of {preview.length} files</p></div><button aria-label="Close final preview" disabled={busy} onClick={()=>setPreview(null)}><X size={18}/></button></header><div className="pdf-preview-files">{preview.map((file,i)=><button className={i===previewIndex?'active':''} key={file.name} data-reviewed={viewed.includes(i)} onClick={()=>setPreviewIndex(i)}>{viewed.includes(i)&&<Check size={13}/>} {file.name}</button>)}</div><label className="pdf-preview-zoom">Preview zoom<select aria-label="Preview zoom" value={previewZoom} onChange={e=>setPreviewZoom(Number(e.target.value))}>{[75,100,125,150,200].map(n=><option value={n} key={n}>{n}%</option>)}</select></label><FinalPdfPreview zoom={previewZoom} file={preview[previewIndex]} onRendering={()=>{setViewed(v=>v.filter(i=>i!==previewIndex));setAck(false);}} onRendered={()=>setViewed(v=>v.includes(previewIndex)?v:[...v,previewIndex])}/><footer><label><input type="checkbox" disabled={viewed.length!==preview.length||busy} checked={ack} onChange={e=>setAck(e.target.checked)}/> I reviewed every output page and acknowledge the visible edits. PDF edits do not change AR balances or OPERA.</label><div className="pdf-preview-actions">{transient&&onContinueToEmail&&<button disabled={!ack||busy||viewed.length!==preview.length} onClick={()=>void save('download')}>Download reviewed PDFs</button>}<button className="pdf-primary" disabled={transient?!ack||busy||viewed.length!==preview.length:canContinue?false:!ack||busy||viewed.length!==preview.length||saved} onClick={transient?()=>void save(onContinueToEmail?'email':'download'):canContinue?onContinueToEmail:()=>void save()}>{transient?(busy?'Preparing files…':onContinueToEmail?'Continue to email':'Download reviewed PDFs'):canContinue?'Continue to email':saved?'Saved':busy?'Saving…':onSave?'Save reviewed PDFs privately':'Download reviewed PDFs'}</button></div></footer></section></div>}

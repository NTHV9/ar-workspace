...
export function PdfWorkspace({ onDirtyChange, transient=false, onReview, documents: sources, initialProject, initialDelivery='combined', initialReviewed=false,onContinueToEmail, accountName, hotel, selectedCount, onClose, onSave, onSaveDraft }: Props) {
  ...
  function commit(next: PdfProject) { if (!project||!sourceModel.current) return; try{next=syncVouchers(project,next,voucherBindings(loaded));}catch(e){setError(e instanceof Error?e.message:'This edit could not be applied.');return;}try{restoreProject(next,sourceModel.current);}catch{setError('This change exceeds safe page, layer, text or image limits and was not applied.');return;} setError('');setPast(p => [...p.slice(-49), project]); setFuture([]); setProject(next); invalidate(); }

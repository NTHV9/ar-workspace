...
export function useNativeEditing({page,documents,runs,layer,tool,setTool,busy,setBusy,onChange,onError}:Props){
 ...
 const row=anchor&&page?rowGeometry(page,anchor,runs):null;
 const cells=row?.cells??[],rowTop=row?.top??0,rowHeight=row?.height??0;
 ...
 async function insertRow(){
  if(!page||!anchor)return;const y=row!.end,tableRow=crypto.randomUUID();
 ...
 async function deleteRow(){if(!page||!anchor)return;const okay=await apply({id:crypto.randomUUID(),kind:'delete',y:rowTop,height:rowHeight},[],row!.members.map(l=>l.id));if(okay)setArea(null);}

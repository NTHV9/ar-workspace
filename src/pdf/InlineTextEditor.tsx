...
export function InlineTextEditor({layer,scale,inputRef,invalid,onChange,onFocus,onNavigate}:{layer:PdfLayer;scale:number;inputRef:RefObject<HTMLTextAreaElement|null>;invalid:boolean;onChange:(value:string)=>void;onFocus:(input:HTMLTextAreaElement)=>void;onNavigate?:(backwards:boolean)=>boolean}){
 ...
 return <textarea ref={inputRef} className={'pdf-inline-text'+(invalid?' needs-font':'')} aria-label="Edit document text" spellCheck={false} autoComplete="off" wrap={layout.lines.length===1?'off':'soft'} value={layer.text} onFocus={event=>onFocus(event.currentTarget)} onChange={event=>onChange(event.target.value)} onKeyDown={event=>{if(event.key==='Tab'&&onNavigate?.(event.shiftKey)){event.preventDefault();event.stopPropagation();}}} onPointerDown={event=>event.stopPropagation()} style={{fontFamily:font.fontFamily,fontSize:font.fontSize*scale,fontWeight:font.fontWeight,fontStyle:font.fontStyle,lineHeight:font.lineHeight,caretColor:layer.color,left:layer.sourceText?0:3*scale,top,width:`calc(${100/textScale}% + 2px)`,whiteSpace:layout.lines.length===1?'pre':'pre-wrap',height:`calc(100% + ${Math.abs(top)+2}px)`,transform:`scaleX(${textScale})`,transformOrigin:'top left'}}/>;
}

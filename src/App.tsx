...
export function App() {
  ...
  const checkOpera=async()=>{
    if(!session)return;setCheckingOpera(true);setProbeResult(null);
    try{
      const results=[];
      for(const hotel of ['KAT','TSK']){
        const response=await fetch(`/api/opera/probe?hotel=${hotel}`,{method:'POST',headers:{Authorization:`Bearer ${session.access_token}`}});
        results.push({hotel,httpStatus:response.status,result:await response.json()});
      }
      setProbeResult(results);
    }catch{setProbeResult({error:'Connection check unavailable. Please retry.'});}
    finally{setCheckingOpera(false);}
  };

...
import {dashboardReportContext,dashboardRemittanceContext,dashboardReturn} from './dashboard/links';
...
export function App() {
  ...
  const updateDashboard=(fields:Record<string,string>)=>{const next=new URLSearchParams(paramsRef.current);for(const [key,value] of Object.entries(fields)){if(!['dashboardDay','dashboardType','dashboardAccount'].includes(key))continue;if(value||key==='dashboardDay')next.set(key,value);else next.delete(key);}paramsRef.current=next;history.pushState(null,'',`?${next}`);setParams(next);};

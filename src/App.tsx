...
import {normalizedDashboardParams,dashboardRemittanceContext,dashboardReturn} from './dashboard/links';
...
export function App() {
  useEffect(()=>{const original=new URLSearchParams(location.search),normalized=normalizedDashboardParams(original);if(normalized.toString()!==original.toString())history.replaceState(null,'','?'+normalized);},[]);
  const [params,setParams]=useState(()=>normalizedDashboardParams(new URLSearchParams(location.search)));
  ...
  const documentDirty=useRef(false);const setDocumentDirty=(value:boolean)=>{documentDirty.current=value;};
  ...
  const update=(key:string,value:string)=>{const next=new URLSearchParams(paramsRef.current);value?next.set(key,value):next.delete(key);if(key==='hotel')next.delete('dashboardPage');paramsRef.current=next;history.replaceState(null,'',`?${next}`);setParams(next);};
  ...
  useEffect(()=>{
    if(!session||review||recovery||writeHold||openedOwner.current===session.user.id)return;
  ...
  const updateDashboard=(fields:Record<string,string>)=>{const next=new URLSearchParams(paramsRef.current);for(const [key,value] of Object.entries(fields)){if(!['dashboardDay','dashboardFrom','dashboardTo','dashboardDateMode','dashboardView','dashboardType','dashboardAccount','dashboardDetail','dashboardMetric','dashboardStage','dashboardPage'].includes(key))continue;if(value||['dashboardDay','dashboardFrom','dashboardTo'].includes(key))next.set(key,value);else next.delete(key);}paramsRef.current=next;history.pushState(null,'',`?${next}`);setParams(next);};
  const navigateView=(view:string)=>{if((templateDirty.current||remittanceDirty.current||accountDirty.current||policyDirty.current||documentDirty.current)&&!window.confirm(documentDirty.current?'Leave this preparation and discard tab-only PDF edits?':'Leave without saving or resolving the current changes?'))return;const next=new URLSearchParams(paramsRef.current);if(view==='dashboard'&&next.get('fromDashboard')==='1')next.set('hotel',next.get('dashboardHotel')??hotel);for(const k of ['dashboard','fromDashboard','dashboardDetail','dashboardStage','dashboardMetric','dashboardPage','dashboardHotel','account','property','documentJob','documents','pdfCheck','pdfHotel','collections','reports','templates','compose','storage','drive','remittances','remitHotel','remitAccount','accountSection','collectionPolicy','financial','externalBilling','observations','operations','acceptance','focusInvoice'])next.delete(k);if(view!=='portfolio')next.set(view,'1');paramsRef.current=next;history.pushState(null,'',`?${next}`);setParams(next);};

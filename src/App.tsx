...
interface ConfigResponse { writeHold?:boolean; documentEditorMaxBytes?:number; googleEnabled?: boolean; supabaseUrl?: string; publishableKey?: string }
...
export function App() {
  ...
  useEffect(()=>{let alive=true;let unsubscribe:(()=>void)|undefined;fetch('/api/config').then(r=>{if(!r.ok)throw new Error();return readJson<ConfigResponse>(r);}).then(config=>{if(!alive)return;setGoogleEnabled(config.googleEnabled===true);setWriteHold(config.writeHold===true);if(typeof config.documentEditorMaxBytes==='number'&&config.documentEditorMaxBytes>0)setDocumentEditorMaxBytes(config.documentEditorMaxBytes);if(!config.supabaseUrl||!config.publishableKey){setAuthReady(true);setState('unavailable');return;}const c=createClient(config.supabaseUrl,config.publishableKey,{auth:{flowType:'pkce',detectSessionInUrl:true}});setClient(c);c.auth.getSession().then(({data})=>{if(alive){setSession(data.session);setAuthReady(true);setState('ready');}});unsubscribe=c.auth.onAuthStateChange((event,s)=>{if(event==='PASSWORD_RECOVERY')setRecovery(true);setSession(s);}).data.subscription.unsubscribe;}).catch(()=>{if(alive){setAuthReady(true);setState('unavailable');}});return()=>{alive=false;unsubscribe?.();};},[]);
  const load=async()=>{
    if(!session)return;

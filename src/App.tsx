...
export function App() {
  ...
  const update=(key:string,value:string)=>{const next=new URLSearchParams(paramsRef.current);value?next.set(key,value):next.delete(key);if(key==='hotel'){next.delete('dashboardPage');next.delete('dashboardDetailHotel');}paramsRef.current=next;history.replaceState(null,'',`?${next}`);setParams(next);};

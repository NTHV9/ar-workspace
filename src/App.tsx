...
interface ConfigResponse { googleEnabled?: boolean; supabaseUrl?: string; publishableKey?: string }
...
export function App() {
  ...
  useEffect(()=>{let alive=true;let unsubscribe:(()=>void)|undefined;fetch('/api/config').then(r=>{if(!r.ok)throw new Error();return readJson<ConfigResponse>(r);}).then(config=>{if(!alive)return;setGoogleEnabled(config.googleEnabled===true);if(!config.supabaseUrl||!config.publishableKey){setState('unavailable');return;}const c=createClient(config.supabaseUrl,config.publishableKey,{auth:{flowType:'pkce',detectSessionInUrl:true}});setClient(c);c.auth.getSession().then(({data})=>{if(alive){setSession(data.session);setState('ready');}});unsubscribe=c.auth.onAuthStateChange((_event,s)=>setSession(s)).data.subscription.unsubscribe;}).catch(()=>{if(alive)setState('unavailable');});return()=>{alive=false;unsubscribe?.();};},[]);
  const load=async()=>{
    if(!session)return;

...
export function App() {
  ...
  const changeRegion=(value:string)=>{
    if(!isRegionId(value)||value===region)return;
  ...
  const visible=(review?demoAccounts:accountsOwner===session?.user.id?accounts:[]).filter(a=>hotelInRegion(a.hotel,region));
  ...
  useEffect(()=>{
    if(!session||review||recovery||writeHold||openedOwner.current===session.user.id+':'+region)return;
    openedOwner.current=session.user.id+':'+region;void requestRefresh('open');
  },[session?.user.id,review,recovery,region]);
  useEffect(()=>{
  ...
  },[session?.access_token,review,recovery,refresh?.running,region]);

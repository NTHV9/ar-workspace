...
export function App() {
  ...
  useEffect(()=>{let alive=true;let unsubscribe:(()=>void)|undefined;fetch('/api/config').then(r=>{if(!r.ok)throw new Error();return readJson<ConfigResponse>(r);}).then(config=>{if(!alive)return;setGoogleEnabled(config.googleEnabled===true);setWriteHold(config.writeHold===true);setAcceptanceSelected(config.acceptanceSelected===true);if(typeof config.documentEditorMaxBytes==='number'&&config.documentEditorMaxBytes>0)setDocumentEditorMaxBytes(config.documentEditorMaxBytes);if(!config.supabaseUrl||!config.publishableKey){setAuthReady(true);setState('unavailable');return;}const c=createClient(config.supabaseUrl,config.publishableKey,{auth:tabAuthOptions(config.supabaseUrl)});setClient(c);c.auth.getSession().then(({data})=>{if(alive){setSession(data.session);setAuthReady(true);setState('ready');}});unsubscribe=c.auth.onAuthStateChange((_event,s)=>{if(alive)setSession(s);}).data.subscription.unsubscribe;}).catch(()=>{if(alive){setAuthReady(true);setState('unavailable');}});return()=>{alive=false;unsubscribe?.();};},[]);
  const load=async()=>{

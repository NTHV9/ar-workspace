...
const UsersAccess=lazy(()=>import('./access/UserAccounts'));
...
export function App() {
  ...
  const [enrolling,setEnrolling]=useState(false),[authNotice,setAuthNotice]=useState('');
  ...
  useEffect(()=>{if(review||!workspaceAccess||workspaceAccess.regions.includes(region))return;const next=new URLSearchParams();next.set('region',workspaceAccess.regions[0]);next.set('hotel','All');paramsRef.current=next;history.replaceState(null,'','?'+next);setParams(next);setAccounts([]);setInvoices([]);loadedCatalog.current=null;},[accessKey,region,review]);
  ...
  const login=async()=>{if(!client)return;setBusy(true);setError('');setAuthNotice('');try{if(enrolling){const {error}=await client.auth.signUp({email:email.trim(),password,options:{emailRedirectTo:location.origin}});if(error)setError('Account creation failed. Check your details and ask your administrator to approve this email.');else setAuthNotice('Check your email to confirm your web app account, then sign in.');}else if(email.includes('@')){const {error}=await client.auth.signInWithPassword({email,password});if(error)setError('Sign-in failed. Check your email and web app password.');}else{const response=await fetch('/api/access/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:email,password})});if(!response.ok){setError(response.status===429?'Too many sign-in attempts. Wait a minute and try again.':'Sign-in failed. Check your username and password.');}else{const value=await response.json() as {access_token?:string;refresh_token?:string};if(typeof value.access_token!=='string'||typeof value.refresh_token!=='string')throw Error();const {error}=await client.auth.setSession({access_token:value.access_token,refresh_token:value.refresh_token});if(error)throw Error();}}}catch{setError('Authentication is unavailable. Please try again.');}finally{setPassword('');setBusy(false);}};

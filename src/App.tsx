...
export function App() {
  ...
  useEffect(()=>{const before=(event:BeforeUnloadEvent)=>{if(accountDirty.current){event.preventDefault();event.returnValue='';}};addEventListener('beforeunload',before);return()=>removeEventListener('beforeunload',before);},[]);
  useEffect(()=>{const pop=()=>{if((templateDirty.current||remittanceDirty.current||accountDirty.current)&&!window.confirm('Leave without saving or resolving the current changes?')){history.pushState(null,'',`?${paramsRef.current}`);return;}const next=new URLSearchParams(location.search);paramsRef.current=next;setParams(next);};addEventListener('popstate',pop);return()=>removeEventListener('popstate',pop);},[]);

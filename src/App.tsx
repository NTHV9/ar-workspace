...
export function App() {
  ...
  useEffect(()=>{const pop=()=>{if(templateDirty.current&&!window.confirm('Leave without saving template edits?')){history.pushState(null,'',`?${paramsRef.current}`);return;}const next=new URLSearchParams(location.search);paramsRef.current=next;setParams(next);};addEventListener('popstate',pop);return()=>removeEventListener('popstate',pop);},[]);
  ...
  const navigateView=(view:string)=>{if(templateDirty.current&&!window.confirm('Leave without saving template edits?'))return;const next=new URLSearchParams(paramsRef.current);for(const k of ['account','property','documentJob','documents','pdfCheck','pdfHotel','collections','reports','templates','compose'])next.delete(k);if(view!=='portfolio')next.set(view,'1');paramsRef.current=next;history.pushState(null,'',`?${next}`);setParams(next);};

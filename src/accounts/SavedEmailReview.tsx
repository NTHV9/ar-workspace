...
export function SavedEmailReview({hotel,accountId,draftId,token,onClose}:{hotel:string;accountId:string;draftId:string;token:string;onClose:()=>void}){
 ...
 return <section className="saved-email-review" aria-label="Saved email review">
  ...
  {saved&&<>
   ...
   <section className="saved-email-conversation" aria-label="Saved linked conversation"><div className="saved-email-heading"><div><h3>Linked conversation</h3><p>Gmail metadata and snippets, checked against the saved recipients and {saved.threadSource==='sent'?'verified sent message':'selected parent'}.</p></div>{saved.hasThread&&<button disabled={threadLoading} onClick={()=>loadConversation(0,true)}>{threadRequested?'Refresh linked conversation':'Load linked conversation'}</button>}</div>
    {!saved.hasThread&&<p>This saved email has no selected conversation or verified sent message available.</p>}{threadLoading&&<p role="status">Reading linked conversation…</p>}{threadError&&<p role="alert" className="error-message">{threadError}</p>}
    {conversation&&<><ol className="saved-conversation-messages">{conversation.messages.map(message=><li key={message.id}><div className="saved-conversation-meta"><strong>{message.direction==='incoming'?'Incoming':message.direction==='outgoing'?'Outgoing':'Direction unknown'}</strong><span>{dateLabel(message.date)}</span></div><p className="saved-conversation-address">{message.from} → {message.to.join(', ')||'Recipient not shown'}</p>{message.matchesReply&&<small className="saved-parent-reference">{saved.threadSource==='sent'?'Reply to the verified sent message':'Reply to the saved parent message'}</small>}<p className="saved-conversation-snippet">{message.snippet||'No snippet is available for this message.'}</p></li>)}</ol><div className="saved-conversation-pagination"><span>{conversation.messages.length?`Messages ${offset+1}–${offset+conversation.messages.length} of ${conversation.thread.messageCount}`:'No messages on this page'}</span><div><button disabled={threadLoading||offset===0} onClick={()=>loadConversation(Math.max(0,offset-50))}>Previous conversation messages</button><button disabled={threadLoading||conversation.nextMessageOffset===null} onClick={()=>loadConversation(conversation.nextMessageOffset!)}>Next conversation messages</button></div></div><p className="saved-conversation-check">Checked {dateLabel(conversation.checkedAt)}</p></>}
   </section>

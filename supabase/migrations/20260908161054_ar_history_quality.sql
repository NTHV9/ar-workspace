-- Preserve history metadata warnings per account and run. Current invoice membership
-- and balances are independently reconciled; historical counts stay unverified.
alter table public.ar_accounts add column "sourceWarnings" jsonb not null default '[]';
create table ar_private.refresh_quality (
  run_id uuid not null references ar_private.refresh_runs(id),
  hotel text not null, account_id text not null, warnings jsonb not null,
  primary key(run_id,hotel,account_id)
);
alter table ar_private.refresh_quality enable row level security;
revoke all on ar_private.refresh_quality from public,anon,authenticated;
grant select,insert on ar_private.refresh_quality to service_role;

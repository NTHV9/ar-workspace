...
create function public.ar_account_workspace_read(p_actor uuid,p_hotel text,p_account text,p_section text,p_offset integer default 0,p_limit integer default 20)
 ...
 if p_section='history' then
  with entries as materialized (
   select 'sent:'||e.delivery_id as id,e.sent_at as recorded_at,'Verified Gmail send' as source,e.purpose,e.stage,
    ...
    jsonb_build_object('subject',m.snapshot->'draft'->>'subject','body',m.snapshot->'draft'->>'body','recipients',m.snapshot->'draft'->'recipients') as message
   ...
   select 'history:'||h.invoice_id||':'||h.revision,h.recorded_at,'Manual history correction','history',h.details->>'last_reminder_stage',
    ...
    (h.details->>'first_billing_date')::date,(h.details->>'last_reminder_date')::date,null::jsonb
   from ar_private.invoice_workflow_history h where h.hotel=p_hotel and h.account_id=p_account and h.actor=p_actor

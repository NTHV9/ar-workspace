-- Read-only current invoice counts. No provider calls, retained snapshots, or money mutation.
create function ar_private.aging_schema_valid(v jsonb) returns boolean language plpgsql immutable set search_path='' as $$
declare b jsonb;s numeric;e numeric;
begin
 if jsonb_typeof(v) is distinct from 'array' or jsonb_array_length(v)=0 then return false;end if;
 for b in select value from jsonb_array_elements(v) loop
  if jsonb_typeof(b) is distinct from 'object' or jsonb_typeof(b->'label') is distinct from 'string' or length(btrim(b->>'label'))=0
   or jsonb_typeof(b->'start') is distinct from 'number' or jsonb_typeof(b->'sequence') is distinct from 'number'
   or jsonb_typeof(b->'amount') is distinct from 'number' or jsonb_typeof(b->'debit') is distinct from 'number' or jsonb_typeof(b->'credit') is distinct from 'number'
   or not(b?'end') then return false;end if;
  s:=(b->>'start')::numeric;e:=(b->>'sequence')::numeric;
  if s<0 or s<>trunc(s) or s>9007199254740991 or e<0 or e<>trunc(e) or e>9007199254740991 then return false;end if;
  if b->'end'<>'null'::jsonb then
   if jsonb_typeof(b->'end') is distinct from 'number' then return false;end if;
   e:=(b->>'end')::numeric;if e<s or e<>trunc(e) or e>9007199254740991 then return false;end if;
  end if;
 end loop;
 if (select count(distinct q.value->'sequence') from jsonb_array_elements(v)q(value))<>jsonb_array_length(v) then return false;end if;
 if exists(select 1 from jsonb_array_elements(v) with ordinality x(b,n) cross join jsonb_array_elements(v) with ordinality y(b,n) where x.n<y.n and (x.b->>'start')::numeric<=coalesce((y.b->>'end')::numeric,9007199254740991) and (y.b->>'start')::numeric<=coalesce((x.b->>'end')::numeric,9007199254740991)) then return false;end if;
 return true;
exception when others then return false;
end$$;
revoke all on function ar_private.aging_schema_valid(jsonb) from public,anon,authenticated,service_role;

create function public.ar_aging_invoice_status(p_actor uuid,p_hotel text default null,p_type text default null,p_kat_account text default null,p_tsk_account text default null,p_bucket jsonb default null,p_dimension text default null,p_status text default null,p_details boolean default false,p_offset integer default 0,p_limit integer default 50,p_flag text default null,p_accounts jsonb default null)
...
begin
 ...
 if p_status is not null and not(case p_dimension when 'billing' then p_status in('unbilled','billed','not_required','setup') when 'due' then p_status in('not_due','due_today','past_due','awaiting_billing','unknown') when 'flags' then p_status in('held','needs_review') when 'followup' then p_status in('none','unknown') or p_status~'^(Friendly|Follow 1|Follow 2|Follow 3|Final|round_[a-z0-9][a-z0-9_-]{0,63})$' else false end) then return jsonb_build_object('error','aging_invalid');end if;
 ...
 coalesce((i.verification_state='verified' or i.verification_state='cleared' and i.open=0) and i.collection_role='child' and exists(select 1 from public.ar_invoices par where par.hotel=i.hotel and par.account_id=i.account_id and par.id=i.parent_invoice_id and par.collection_role='parent' and par.parent_invoice_id is null and (par.verification_state='verified' or par.verification_state='cleared' and par.open=0)),false) as child,
 ...
 exists(select 1 from public.ar_refresh_state rs where rs.hotel=a.hotel and rs.last_success_at is not null) and (a.verification_state='verified' or a.verification_state='cleared' and a.open=0) and q.unverified=0 and (m.bucket is null or a.schema_valid and q.unassigned=0) as complete,
 ...
 'billingStatus',case when s.billing_required is null then 'setup' when not s.billing_required then 'not_required' when s.first_billing_date<=d then 'billed' else 'unbilled' end,
 'latestStage',coalesce(s.stage,'none'),'latestStageLabel',coalesce(s.stage_label,'No Follow-Up'),
 'dueStatus',case when s.billing_required and (s.first_billing_date is null or s.first_billing_date>d) then 'awaiting_billing' when s.due_date is null then 'unknown' when s.due_date<d then 'past_due' when s.due_date=d then 'due_today' else 'not_due' end,'dueDate',s.due_date,'held',s.held,'needsReview',s.needs_review)),'[]') from source s where s.root and s.account_verified and s.open>0 and (p_bucket is null or s.schema_valid and s.matches=1 and s.bucket=p_bucket)) into account_rows,source_rows;
 ...
 keys as(select * from(values('billing','unbilled','Not billed'),('billing','billed','Billed'),('billing','not_required','Billing not required'),('billing','setup','Billing setup needed'),('due','not_due','Not due'),('due','due_today','Due today'),('due','past_due','Past Due date'),('due','awaiting_billing','Awaiting billing'),('due','unknown','Due date unavailable'),('flags','held','On hold'),('flags','needs_review','Needs review'),('followup','none','No Follow-Up'),('followup','Friendly','Friendly'),('followup','Follow 1','Follow-Up 1'),('followup','Follow 2','Follow-Up 2'),('followup','Follow 3','Follow-Up 3'),('followup','Final','Final'))k(dimension,key,label)
 union all select 'followup',key,label from(select distinct on(r->>'latestStage') r->>'latestStage' as key,r->>'latestStageLabel' as label from rows where r->>'latestStage' not in('none','Friendly','Follow 1','Follow 2','Follow 3','Final') order by r->>'latestStage',r->>'latestStageLabel') custom),
 ...
 'billing',(select jsonb_agg(jsonb_build_object('key',key,'label',label,'count',count,'amount',amount) order by array_position(array['unbilled','billed','not_required','setup'],key)) from facets where dimension='billing'),
 'followup',(select jsonb_agg(jsonb_build_object('key',key,'label',label,'count',count,'amount',amount) order by coalesce(array_position(array['none','Friendly','Follow 1','Follow 2','Follow 3','Final'],key),99),key) from facets where dimension='followup'),
 'due',(select jsonb_agg(jsonb_build_object('key',key,'label',label,'count',count,'amount',amount) order by array_position(array['not_due','due_today','past_due','awaiting_billing','unknown'],key)) from facets where dimension='due'),

...
begin
 ...
 if found then
  if p_mode='test' and (m.mode<>'test' or m.snapshot->'expected'->>'recipientHash' is distinct from p_expected->>'recipientHash' or m.snapshot->'expected'->'supplementalSource' is distinct from p_expected->'supplementalSource') then return jsonb_build_object('error','email_test_command_conflict');end if;

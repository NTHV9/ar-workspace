-- Each comparison group keeps total and hotel values in one stable database snapshot.
-- Independent groups no longer wait for every financial reader. No new stored data.
create function public.ar_dashboard_region_segment(p_actor uuid,p_from date,p_to date,p_type text,p_region text,p_segment text)
...
begin
 ...
 if p_segment is null or p_segment not in('balances','activity','external','entries','payments','paid') then return jsonb_build_object('error','dashboard_invalid');end if;
 ...
 foreach h in array array_prepend(region_scope,ar_private.report_scope_hotels(region_scope)) loop
  scope:=jsonb_build_object('balances',null,'activity',null,'external',null,'entries',null,'payments',null,'paid',null);
  foreach segment in array array[p_segment] loop
   part:=null;
   begin
    case segment
...
revoke all on function public.ar_dashboard_region_segment(uuid,date,date,text,text,text) from public,anon,authenticated,service_role;
grant execute on function public.ar_dashboard_region_segment(uuid,date,date,text,text,text) to service_role;

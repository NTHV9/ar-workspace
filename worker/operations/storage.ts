import {acceptanceRpc,workingBucket,acceptanceLimits} from '../acceptance/routing';
...
export async function readManagedStorage(env:StorageBudgetEnv,key:string,ceiling:number,options:{headers?:HeadersInit;timeoutMs?:number;sizeError?:string}={}):Promise<Response>{
 ...
 let expected=ceiling;if(env.OPERATIONS_BUDGET_ENABLED==='true'){
  const receipt=await rpc<{byteCount:number}>(env,'ar_storage_read_budget',{p_actor:await actor(env),p_id:crypto.randomUUID(),p_key:key,p_ceiling:ceiling,p_limits:operationBudgetLimits(acceptanceLimits(env))});
 ...
 const response=await fetch(base(env,false)+'/storage/v1/object/authenticated/'+workingBucket(env)+'/'+key,{headers:options.headers??{apikey:env.SUPABASE_SECRET_KEY!},redirect:'manual',signal:AbortSignal.timeout(options.timeoutMs??30000)});

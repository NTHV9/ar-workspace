...
import {INVOICE_TEMPLATE_VERSION,type InvoiceAssets} from './types';

export async function workspaceInvoice(env:RefreshEnv,job:DocumentJob,invoice:DocumentInvoice){
 if(job.invoice_source!=='workspace'||job.invoice_template_version!==INVOICE_TEMPLATE_VERSION)throw Error('document_invoice_source_invalid');

...
import {closeAcceptance} from './closeout';
import {sweepRetention,sweepTransientDocuments} from '../operations/retention-sweep';
...
export async function acceptanceApi(request:Request,env:RefreshEnv&DriveEnv,actor:string){
 ...
 try{
  ...
  const id=acceptanceCookie(request);if(!id)throw Error('acceptance_inactive');
  if(path==='/api/acceptance/close'&&data.confirmed===true){return json(await closeAcceptance(env,actor,id),200,{'Set-Cookie':acceptanceCookieHeader(null)});}
  const scoped=await acceptanceEnvironment(env,actor,id);
  ...
  if(path==='/api/acceptance/retention'){
   const retentionEnv={...scoped,RETENTION_ENABLED:'true'};
   const transient=await sweepTransientDocuments(retentionEnv);
   return json({...await sweepRetention(retentionEnv),transient});
  }

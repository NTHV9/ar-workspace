...
export async function settingsApi(request:Request,env:RefreshEnv,actor:string){try{
 ...
 let result;if(parts[2]==='account-settings'&&parts.length===5){
  ...
  else{const v=parseSettings(await input(request));result=await rpc(env,'ar_settings_save',{p_actor:actor,p_hotel:hotel,p_account_id:accountId,p_revision:v.revision,p_billing_required:v.billingRequired,p_credit_term:v.creditTerm,p_billing_recipients:v.billingRecipients,p_collection_recipients:v.collectionRecipients});}
  ...
  if(!Number.isSafeInteger(v.revision)||v.revision<0||!date(v.firstBillingDate)||!date(v.lastReminderDate)||!(v.lastReminderStage===null||['Friendly','Follow 1','Follow 2','Follow 3','Final'].includes(v.lastReminderStage))||!invoiceId||invoiceId.length>200)return json({error:'history_invalid'},400);
 ...
 if(typeof result?.error==='string')return json(result,/conflict/.test(result.error)?409:/forbidden/.test(result.error)?403:/missing/.test(result.error)?404:400);return json(result);

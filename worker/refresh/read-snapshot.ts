...
export async function readVerifiedAccount(reader:OperaReader,hotel:string,accountId:string,businessDate:string,previous:PreviousInvoice[]=[]):Promise<AccountSnapshot> {
  ...
  const readAuditHistory=async(includeZero:boolean)=>{
    let oldBalanceRows=0;const oldBalanceForms:Record<string,number>={};let firstPageRows=0;
    ...
    }catch(error){if(error instanceof OperaError)throw new OperaError(error.code,error.upstreamStatus,error.stage,error.providerMessage,{...error.diagnostics,oldBalanceRows,includeZero,firstPageRows,oldBalanceForms:JSON.stringify(oldBalanceForms)});throw error;}
  };

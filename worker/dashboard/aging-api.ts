...
export function parseAgingInvoicesQuery(url:URL){
 ...
 const regional=(()=>{try{return regionalHotelScope(q);}catch{return invalid();}})();
 if(regional.region==='khao-lak'&&(kat||tsk)||regional.hotel==='KAT'&&tsk||regional.hotel==='TSK'&&kat||[type,kat,tsk].some(v=>v&&v.length>200)||details&&details!=='1')invalid();
 ...
 return {p_hotel:regional.reportHotel,p_type:type,p_kat_account:kat,p_tsk_account:tsk,p_bucket:bucket,p_dimension:dimension,p_status:status,p_details:details==='1',p_offset:page*limit,p_limit:limit,p_flag:flag,p_accounts:accounts};
}

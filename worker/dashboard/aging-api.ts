...
export function parseAgingInvoicesQuery(url:URL){
 ...
 if(status&&(dimension==='followup'?!['none','unknown'].includes(status)&&!isCollectionStageKey(status):!(agingStatusKeys[dimension as keyof typeof agingStatusKeys] as readonly string[]).includes(status)))invalid();

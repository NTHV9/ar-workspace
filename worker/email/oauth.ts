...
export async function gmailConnect(env:EmailEnv,owner:string,job:string){
 ...
 const url=new URL('https://accounts.google.com/o/oauth2/v2/auth');url.search=new URLSearchParams({client_id:env.GMAIL_CLIENT_ID!,redirect_uri:callback,response_type:'code',scope:gmailScope,access_type:'offline',prompt:'consent',login_hint:'ar@katathani.com',state,code_challenge:challenge,code_challenge_method:'S256'}).toString();
...
export async function gmailCallback(request:Request,env:EmailEnv){let job='';const redirect=(status:string)=>new Response(null,{status:303,headers:{Location:origin+'/?'+new URLSearchParams({...job?{documentJob:job,compose:'1'}:{},gmail:status}),'Set-Cookie':'__Host-ar_gmail_state=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0','Cache-Control':'no-store','Referrer-Policy':'no-referrer'}});
 ...
 if(typeof tokens.access_token!=='string'||typeof tokens.refresh_token!=='string'||typeof tokens.expires_in!=='number'||typeof tokens.scope!=='string'||!tokens.scope.split(' ').includes(gmailScope))throw Error();
...
export async function gmailStatus(env:EmailEnv,owner:string){if(!gmailConfigured(env))return {configured:false,connected:false};try{const token=await gmailToken(env,owner);const p=await googleJson('https://gmail.googleapis.com/gmail/v1/users/me/profile',{headers:{Authorization:'Bearer '+token}});return {configured:true,connected:p.emailAddress==='ar@katathani.com',email:p.emailAddress==='ar@katathani.com'?p.emailAddress:null};}catch{return {configured:true,connected:false};}}

...
export async function templateRequest(request:Request,env:EmailEnv,actor:string):Promise<Response>{
 try{
  ...
  if(request.method==='GET'){
   ...
   const v=await jsonBody(request);if(Object.keys(v).some(k=>!['revision','content'].includes(k))||!Number.isSafeInteger(v.revision)||Number(v.revision)<0)return emailJson({error:'template_invalid'},400);
   result=await emailRpc(env,'ar_template_save',{p_actor:actor,p_id:match[1],p_revision:v.revision,p_content:parseTemplate(v.content)});
  }else return emailJson({error:'method_not_allowed'},405);
  if(result&&typeof result==='object'&&'error' in result){const code=String(result.error);return emailJson({error:code},code==='template_revision_conflict'?409:code==='template_missing'?404:code==='email_forbidden'?403:400);}

...
export function buildMime(input:EmailInput,files:MailFile[],messageId:string):Uint8Array{
 ...
 const headers=['From: ar@katathani.com',...(['to','cc','bcc'] as const).filter(f=>v.recipients[f].length).map(f=>`${f}: ${v.recipients[f].join(',\r\n ')}`),'Subject: '+encodedHeader(v.subject),'Message-ID: '+messageId,'MIME-Version: 1.0',`Content-Type: multipart/mixed; boundary="${boundary}"`];
 const parts=[headers.join('\r\n'),'','--'+boundary,'Content-Type: text/plain; charset=UTF-8','Content-Transfer-Encoding: base64','',lines(base64(new TextEncoder().encode(v.body)))];

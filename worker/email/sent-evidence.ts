...
export async function verifySentEvidence(message:Message,expected:ExpectedMail,attachment:(id:string)=>Promise<Uint8Array>):Promise<{status:'verified'|'not_sent'|'review_required';sentAt?:string}>{
 ...
 try{
  ...
  if(normalizedText(new TextDecoder().decode(decodeUrl64(plain[0].body.data)))!==normalizedText(expected.body))throw Error();
  return {status:'verified',sentAt:new Date(date).toISOString()};
 }catch{return {status:'review_required'};}
}

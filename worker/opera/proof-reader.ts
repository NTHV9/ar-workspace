...
export interface ProofReadStats {calls:number;joinedQueued:number;peakActive:number;ioMs:number;queueMs:number;failures:number}
...
export function createFinancialProofReader(source:FinancialProofReader){
 const counters:ProofReadStats={calls:0,joinedQueued:0,peakActive:0,ioMs:0,queueMs:0,failures:0};
 type Job={key:string;run:()=>Promise<unknown>;resolve:(value:unknown)=>void;reject:(error:unknown)=>void;queuedAt:number};
 const queue:Job[]=[],waiting=new Map<string,Promise<unknown>>();let active=0,scheduled=false;
 const drain=()=>{
  scheduled=false;
 ...
 const wrap=<A extends unknown[]>(name:string,read:(...args:A)=>Promise<unknown>)=>async(...input:A)=>{
  ...
  if(pending)counters.joinedQueued++;
  else{
   pending=new Promise<unknown>((resolve,reject)=>queue.push({key,run:()=>read(...args),resolve,reject,queuedAt:Date.now()}));waiting.set(key,pending);

...
export async function accessApi(request:Request,env:RefreshEnv,actor:string,email:string):Promise<Response>{try{
 ...
 if(Object.keys(input).some(k=>!['commandId','email','regions','active','revision'].includes(k))||typeof input.commandId!=='string'||!uuid.test(input.commandId)||typeof input.email!=='string'||input.email.length>254||!Array.isArray(input.regions)||input.regions.length<1||input.regions.length>2||input.regions.some(r=>!isRegionId(r))||new Set(input.regions).size!==input.regions.length||typeof input.active!=='boolean'||!Number.isSafeInteger(input.revision)||Number(input.revision)<0)throw Error('access_invalid');
 return json(await backendRpc(env,'ar_access_save',{p_actor:actor,p_command:input.commandId,p_email:input.email,p_regions:input.regions,p_active:input.active,p_revision:input.revision}));

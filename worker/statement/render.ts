...
export const supportedStatementTemplate=(version:string|null|undefined)=>version==='rtf-20260909-v3'||version==='rtf-20260921-v4';
...
export async function renderStatement(model:StatementModel,assets:StatementAssets):Promise<Uint8Array> {
 if(assets.hotel!==model.hotel||!supportedStatementTemplate(assets.version)||!model.rows.length||model.rows.length>500)fail();
 ...
 const footerHeight=assets.version==='rtf-20260909-v3'?77:assets.footer.height;
 const contentBottom=Math.min(705,792-footerHeight-10);
 ...
 add(true);
 for(const r of model.rows){const values=[r.date,r.folio,r.guest,r.arrival,r.departure,r.voucher.trim(),money(r.debit),r.credit?money(r.credit):'',money(r.balance)];const cells=values.map((s,i)=>wrap(s,edges[i+1]-edges[i]-8));if(cells.slice(6).some(c=>c.length!==1))fail();const h=Math.max(...cells.map(c=>c.length))*10+4;if(y+h>contentBottom)add(true);if(y+h>contentBottom)fail();addVoucherField(page,{hotel:model.hotel,accountId:model.accountId,invoiceId:r.id,align:'center',text:r.voucher.trim(),x:332,y:y-.5,width:74,height:h-1,fontSize:8,bold:false});cells.forEach((lines,i)=>lines.forEach((s,j)=>write(s,i>=6?edges[i+1]-4-width(s,8):i===5?(edges[i]+edges[i+1]-voucherWidth(s))/2:edges[i]+4,y+j*10)));y+=h;}
 if(y+27>contentBottom)add(true);rectangle(32,y,548,26,.85);write('Balance Due',400,y+8,8,true);const total=money(model.total)+' (THB)';if(bold.widthOfTextAtSize(total,8)>120)fail();write(total,576-bold.widthOfTextAtSize(total,8),y+8,8,true);y+=40;
 const closingHeight=60+assets.closing.height;if(y+closingHeight>contentBottom)add(false);if(y+closingHeight>contentBottom)fail();write('Aging Summary:',36,y,9,true);const tableY=y+14;for(let i=0;i<6;i++){const x=32+i*548/6;rectangle(x,tableY,548/6,17,.85);rectangle(x,tableY+17,548/6,17);const label=model.aging[i].label,value=money(model.aging[i].cents);if(width(label,8,true)>548/6-4||width(value,8)>548/6-4)fail();write(label,x+(548/6-width(label,8,true))/2,tableY+4,8,true);write(value,x+(548/6-width(value,8))/2,tableY+21);}

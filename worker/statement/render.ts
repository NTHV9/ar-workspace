...
export async function renderStatement(model:StatementModel,assets:StatementAssets):Promise<Uint8Array> {
 ...
 add(true);
 for(const r of model.rows){const values=[r.date,r.folio,r.guest,r.arrival,r.departure,r.voucher.trim(),money(r.debit),r.credit?money(r.credit):'',money(r.balance)];const cells=values.map((s,i)=>wrap(s,edges[i+1]-edges[i]-8));if(cells.slice(6).some(c=>c.length!==1))fail();const h=Math.max(...cells.map(c=>c.length))*10+4;if(y+h>contentBottom)add(true);if(y+h>contentBottom)fail();cells.forEach((lines,i)=>lines.forEach((s,j)=>write(s,i>=6?edges[i+1]-4-width(s,8):i===5?(edges[i]+edges[i+1]-voucherWidth(s))/2:edges[i]+4,y+j*10)));y+=h;}

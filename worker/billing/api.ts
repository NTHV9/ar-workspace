...
import {regionalHotelScope,resultMatchesHotelScope,type RegionalHotelScope} from '../hotels';
export type ExternalBillingInput={commandId:string;action:'record'|'correct'|'void'|'restore';hotel:HotelId;accountId:string;recordId?:string;revision?:number;actualDate?:string;channel?:'system'|'external_email';reference?:string;note?:string;amount?:string|null;lines?:{invoiceId:string;revision:number}[];reason?:string};
...
export function parseExternalBilling(value:unknown):ExternalBillingInput{
 ...
 if(typeof v.commandId!=='string'||!uuid.test(v.commandId)||!['record','correct','void','restore'].includes(String(v.action))||!isHotelId(v.hotel)||!text(v.accountId,200))return invalid();
 if(v.action==='record'){
...
export async function externalBillingApi(request:Request,env:RefreshEnv,actor:string){
 try{
  if(!uuid.test(actor))return json({error:'billing_forbidden'},403);const url=new URL(request.url);let result:Record<string,unknown>,responseScope:RegionalHotelScope|null=null,writeHotel:HotelId|null=null;
  const historyPath=/^\/api\/external-billing\/([0-9a-f-]{36})\/history$/.exec(url.pathname);
  if(request.method==='GET'&&historyPath){for(const key of url.searchParams.keys())if(!['page','region'].includes(key)||url.searchParams.getAll(key).length!==1)return invalid();const page=Number(url.searchParams.get('page')??0);try{responseScope=regionalHotelScope(url.searchParams);}catch{return invalid();}if(!uuid.test(historyPath[1])||!Number.isSafeInteger(page)||page<0||page>1000000)return invalid();result=await backendRpc(env,'ar_external_billing_history',{p_actor:actor,p_id:historyPath[1],p_offset:page*20,p_limit:20});
  }else if(request.method==='GET'&&url.pathname==='/api/external-billing'){
   const q=url.searchParams;for(const key of q.keys())if(!['region','hotel','account','type','from','to','page'].includes(key)||q.getAll(key).length!==1)return invalid();
   ...
   try{responseScope=regionalHotelScope(q,account);}catch{return invalid();}
   if(account&&account.length>200||!date(from)||!date(to)||from&&to&&from>to||!Number.isSafeInteger(page)||page<0||page>1000000)return invalid();
   result=await backendRpc(env,'ar_external_billing_read',{p_actor:actor,p_hotel:responseScope.reportHotel,p_account:account,p_from:from,p_to:to,p_offset:page*50,p_limit:50,p_type:type});
   ...
   if(!raw||typeof raw!=='object'||Array.isArray(raw))return invalid();const body=raw as Record<string,unknown>;if(Object.keys(body).some(k=>!['input','digest'].includes(k)))return invalid();const input=parseExternalBilling(body.input);writeHotel=input.hotel;
  ...
  if(!result||typeof result!=='object')throw Error('billing_unavailable');if(typeof result.error==='string')throw Error(/^billing_[a-z_]+$/.test(result.error)?result.error:'billing_unavailable');
  if(responseScope&&!resultMatchesHotelScope(result,responseScope.region,responseScope.hotel)||writeHotel&&!resultMatchesHotelScope(result,hotelRegion(writeHotel),writeHotel))throw Error('billing_unavailable');
  return json(result);

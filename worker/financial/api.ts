...
type Status={running:boolean;runs:{id:string;hotel?:HotelId;status:string;stepsVersion?:number}[]};
async function currentStatus(env:FinancialIngestionEnv,actor:string,region:RegionId|null){
 const expectedRegion=region??'phuket';
 const read=async()=>{const result=await backendRpc<Status>(env,region?'ar_financial_region_status':'ar_financial_status',region?{p_actor:actor,p_region:region}:{p_actor:actor});if(!result||!Array.isArray(result.runs)||!resultMatchesHotelScope(result,expectedRegion))throw Error('financial_unavailable');return result;};
...
export function financialFilters(url:URL){
 const q=url.searchParams;for(const key of q.keys())if(!['region','hotel','account','type','from','to','page','limit'].includes(key)||q.getAll(key).length!==1)throw Error('financial_invalid');
 ...
 let regional;try{regional=regionalHotelScope(q,account);}catch{throw Error('financial_invalid');}
 if((from===null)!==(to===null)||!date(from)||!date(to)||from&&to&&(from>to||(Date.parse(to)-Date.parse(from))/86400000>3660))throw Error('financial_invalid');
 ...
 return {p_hotel:regional.reportHotel,p_account:account,p_type:type,p_from:from,p_to:to,p_offset:page*limit,p_limit:limit};
}
export async function financialApi(request:Request,env:FinancialIngestionEnv,actor:string){
 try{
  ...
  if(url.pathname==='/api/financial/status'&&request.method==='GET'){
   for(const key of url.searchParams.keys())if(key!=='region'||url.searchParams.getAll(key).length!==1)throw Error('financial_invalid');
   let regional;try{regional=regionalHotelScope(url.searchParams);}catch{throw Error('financial_invalid');}
   return json(await currentStatus(env,actor,regional.explicitRegion?regional.region:null));
  }
  ...
  const filters=financialFilters(url),scope=regionalHotelScope(url.searchParams,filters.p_account);
  const data=await backendRpc<FinancialReport>(env,'ar_financial_report',{p_actor:actor,p_view:view,...filters});
  if(!data||!Array.isArray(data.rows)||!Number.isSafeInteger(data.total)||!data.summary||!data.coverage||!resultMatchesHotelScope(data,scope.region,scope.hotel))throw Error('financial_unavailable');return json(data);

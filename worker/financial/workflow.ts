...
export class ArFinancialWorkflow extends WorkflowEntrypoint<FinancialIngestionEnv,RefreshParams>{
 async run(event:WorkflowEvent<RefreshParams>,step:WorkflowStep){
  ...
  if(result.status==='succeeded'){try{await step.do('period-summary-precompute',{retries:{limit:0,delay:'5 seconds'},timeout:'5 minutes'},()=>warmPeriodSummaries(runtime,hotelRegion(payload.hotel)));}catch{/* Published source data remains successful; maintenance retries this optional cache. */}}
  return result;
 }
}

...
export async function driveApi(request:Request,env:DriveEnv,actor:string):Promise<Response>{
 try{
  ...
  if(path==='/api/drive/status'&&request.method==='GET'){
   const t=await target(env,actor),base={configured:driveConfigured(env),connected:false,email:null,folder:folderView(t),cleanupDisabled:true,pickerConfigured:pickerConfigured(env)};
  ...
  if(job&&uuidPattern.test(job[1])&&request.method==='GET'){const value=u.searchParams.get('revision'),revision=Number(value);if(value===null||!Number.isSafeInteger(revision)||revision<0)throw Error('drive_invalid');const a=await driveRpc<Archive|null>(env,'ar_drive_archive_get',{p_owner:actor,p_job:job[1],p_revision:revision});return json({archive:a?archiveView(a):null});}
  if(request.method==='POST'&&(path==='/api/drive/test'||(job&&uuidPattern.test(job[1])))){
   ...
   const t=await target(env,actor);if(!t)throw Error('drive_target_missing');if(t.revision!==v.expectedTargetRevision)throw Error('drive_target_changed');if(!t.verified_at)throw Error('drive_target_not_ready');const token=await driveToken(env,actor);await identity(token);
   ...
   const updated=await runArchive(env,token,a);return json({archive:archiveView(updated)});
  }

...
| C4 | Account history/external billing: account evidence API/UI, audit projection | Exact Hotel/Account paginated billing/reminder/correction history; By System link and actual staff-recorded date/reference; daily external activity distinct from Gmail | Deployed; actual By System/manual history and final isolated scenario passed |
| C5 | Financial history: worker/opera/financial-history, coverage tables/workflow, reports | Date-bounded complete history including unseen zero invoices; minimal signed receipt fields; validated current applications; date semantics explicit; atomic coverage publication | Both hotels published; KAT v2 5752/369/1836 and TSK1001/135/265 invoice/payment/application rows. Isolated queue deployed40a940e; three-invoice financial scenario and report acceptance passed |
| C6 | Reports: historical entries, zero/reopen observations, daily captured AR/aging, durations/over60-unbilled | Filters/drilldowns reconcile; initial import not today; observed timestamps not financial event dates; receipt and application metrics distinct | Deployed40a940e; browser verified, actual daily captures2/175Account rows. Final isolated scenario passed |
| C7 | Account Documents & Gmail and Operations exception access | Reach existing jobs/threads, inspect uncertain handoffs, safe reviewed outcomes without resend shortcut | Completed; see FINAL_ACCEPTANCE_20260911.md |
| C8 | Global budget, retention and storage UI: operations, additive schema, upload/read integration | Atomic concurrent reservations; unknown usage blocks new costly work; exact eligible-file inventory; one-month policy tested; unrelated/pending/reopened/shared files protected | Budget and retention enabled. Actual provider proof removed20 test Supabase objects/3 Drive files after the isolated one-month clock; history retained, then the disposable workspace was removed |
| C9 | Recovery: migration filename alignment, included backup inventory, isolated restore and recovery write hold | No production restore; verify DB/object/provider boundaries; safe replay and post-backup Gmail reconciliation runbook; spend cap checked | 59-migration/19fixture synthetic pg_dump/pg_restore and60-migration schema replay passed. All filenames aligned to hosted versions. Write hold and actual SENT metadata audit verified |
| C10 | Mobile companion/dirty forms/Auth recovery | Desktop PDF editing boundary; narrow screens and keyboard; settings survive refresh; explicit discard; password recovery actual configuration checked | Implemented/deployed; Google/password login verified earlier and recovery PKCE/form tests passed. Actual password change was deliberately not performed without the owner |
| C11 | Final acceptance, privacy review, push/deploy/runbook | Typecheck/build/unit/SQL/browser/security; synthetic screenshots1440/1280/390; SHA/deployment match; no real customer evidence in Git | Verified and delivered; see FINAL_ACCEPTANCE_20260911.md |
...
## Owner-added final end-to-end acceptance — 11 September 2026

After required implementation is finished, create one clearly isolated synthetic Account with three synthetic Invoices in this app and exercise the real application workflow across its implemented parts. Fix discovered bugs and repeat the affected flows. Keep synthetic data out of normal business reports/OPERA ingestion. Do not create or modify OPERA ledger accounts/invoices. Native Invoice API validation remains a separate authorized read-only source check; synthetic invoice documents must be identified as test fixtures.

Use the already authorized ephemeral test recipient from the conversation only for synthetic-content email tests. Do not add it to committed examples, permanent production recipients or reports. Track exact created Account/Invoice/job/object/Drive IDs in a private scenario manifest. After verification, remove only the scenario Account and its scoped test data/artifacts; preserve all real business data and required non-sensitive audit evidence. Build cleanup eligibility from exact scenario ownership, not name patterns. Completion evidence must describe which legs used actual providers and which used synthetic source inputs, without implying fake invoice data came from OPERA.

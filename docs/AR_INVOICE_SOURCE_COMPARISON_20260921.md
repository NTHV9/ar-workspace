...
The normal generator is not changed to a guessed selector, and no title or balance is overwritten. The owner subsequently proposed using the same approach as Statement, pointing to the local Downloads/Folio directory. Inspection found six RTF templates, not exported customer PDFs. The new direction is a system-rendered Invoice using each hotel's supplied template and verified OPERA JSON data. This is not a manual PDF-import workflow and does not remove OPERA as the financial source. Production source switching has not been implemented or enabled.
...
Remaining work: verify the authoritative charge/payment/tax inputs and their exact invoice scope; adapt the layout for the existing Worker-compatible renderer with private hotel assets; validate credit, partial payment, adjustment and multi-page cases; compare same-data output against the two supplied native references; integrate new-document source/provenance without rewriting existing preparations. No template or customer values are committed.

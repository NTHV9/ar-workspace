...
## Native PDF trial — 2026-09-09

The standalone folioHistory GET did not work in this environment: reservationIdId returned RSV00060; root id reached FOF00404 (Voiding Folios functionality not active). Do not enable OPERA accounting controls to bypass this. Reservation-scoped getFolio with includeFolioHistory=true and fetchInstructions Reservation/Foliohistory returned the matching historical Folio and real folioWindowNo for both samples. The response identity and exact invoiceNo/folioNo were matched before report rendering.

getFolioReport required the reservationIdContext=OPERA and reservationIdType=Reservation parameters used by Oracle's [official Postman collection](https://github.com/oracle/hospitality-api-docs/blob/main/postman-collections/property/oracle-hospitality-property.postman_collection.json). Without that context the trial returned500; with it, explicit verified window/date requests returned actual PDFs for both hotels. Files and expected identity sidecars are private. The one-page Copy of Invoice templates print Folio No.; the internal AR invoiceNo was not found in PDF text. Keep API identity lineage rather than claiming text-only verification. Folio footer balance can be zero after City Ledger transfer while AR is still open; never clear AR from that footer.

Selected A/C statement data requests returned exactly the two selected transactions and matching selected balance, excluding B, for both hotel trials. Native selected-only Statement PDF and larger/multi-page/non-reservation document coverage remain unverified. Report rendering has no automatic retries and is limited to explicit private validation jobs. No statement-generation POST, email send or financial mutation was performed.

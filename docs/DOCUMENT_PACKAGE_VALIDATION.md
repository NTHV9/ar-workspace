...
3. **Development watcher instability:** a retained Vite process reported an unhandled `EBUSY` while watching a private downloaded PDF. Excluded `private` and `.cache` from development file watching; source/test directories remain watched. The full 20-case mixed browser run then passed from startup in 50.3 seconds, without increasing timeouts or weakening assertions. This development-only setting does not change the deployed Worker runtime.

## Test execution and deployment

- Final implementation: 156 unit tests, Typecheck, Vite build and connector bundle passed.
- Final browser run: all 20 cases passed together (seven using deployed frontend assets with synthetic/intercepted business API responses, thirteen local Editor harness cases). Includes viewport checks at 1440×900 and 1280×800. These synthetic tests do not substitute for the real-data checks above.
- Earlier mixed cold-start runs encountered localhost navigation timeouts. An experimental Vite entry-list change did not resolve them and was reverted. Later process output exposed the private-PDF watcher crash; the watch exclusion above resolved the observed mixed-run failure. Historical failed logs remain private; no assertion or timeout was weakened.
